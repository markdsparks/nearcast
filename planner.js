...
function writePlanWatchNotificationPreference(value) {
  try {
    ...
    publishLegacyPlanHandoverSnapshot();
  } catch {
...
function markPlanWatchMemoryInventoryReady() {
  ...
  // Plan memory has now completed its one authoritative legacy hydration.
  // The Agenda export is intentionally separate from watch inventory and
  // notification preferences; it reads only the plan-memory record below.
...
function savePlanMemories() {
  ...
  try {
    ...
    // The durable plan record is the only Agenda source. Publish it before
    // unrelated watch-sync bookkeeping so an Agenda never depends on a
    // notification preference or registration state.
...
/* ---------- Native Plans handover rehearsal (read-only) ---------- */

// This is deliberately a separate contract from the visual Agenda export.
// Agenda can be safely useful before native owns plan mutation; the handover
// rehearsal needs complete, exact notification intent as well, and therefore
// refuses to publish until the legacy inventory is known. It never includes
// APNs credentials, registration IDs, continuity receipts, or server state.
const LEGACY_PLAN_HANDOVER_EXPORT_VERSION = 1;
const LEGACY_PLAN_HANDOVER_MAX_BYTES = 160 * 1024;
const LEGACY_PLAN_HANDOVER_MAX_SELECTED_PLANS = 3;
const LEGACY_PLAN_HANDOVER_MAX_SELECTED_PLACES = 3;

function legacyPlanHandoverSafeID(value) {
  return typeof value === "string" && legacyAgendaValidText(value, 160, true);
}

function legacyPlanHandoverReadJSON(key) {
  try {
...
function legacyPlanHandoverNotificationIntent(plans) {
  // `planWatchInventoryIsKnown` is intentionally stronger than a successful
  // Plan read. It prevents an early native bridge callback from turning a
  // partially hydrated selection into "notifications off".
  if (!planWatchInventoryIsKnown()) return null;

  let rawPreference;
  try { rawPreference = localStorage.getItem(PLAN_WATCH_NOTIFICATION_PREF_KEY); }
  catch { return null; }
  const globalPreference = rawPreference === null ? "off" : rawPreference;
  if (!["enabled", "off", "disabled"].includes(globalPreference)) return null;

  const rawPlanPreferences = legacyPlanHandoverReadJSON(PLAN_WATCH_NOTIFICATION_PLANS_KEY);
  if (!rawPlanPreferences.known) return null;
  const planPreferenceObject = rawPlanPreferences.value || { plans: {} };
  if (!planPreferenceObject || typeof planPreferenceObject !== "object" || Array.isArray(planPreferenceObject) ||
      Object.keys(planPreferenceObject).length !== 1 || !Object.hasOwn(planPreferenceObject, "plans") ||
      !planPreferenceObject.plans || typeof planPreferenceObject.plans !== "object" || Array.isArray(planPreferenceObject.plans)) return null;
  const selectedPlanIDs = Object.entries(planPreferenceObject.plans)
    .filter(([id, enabled]) => legacyPlanHandoverSafeID(id) && typeof enabled === "boolean" && enabled)
    .map(([id]) => id)
    .sort();
  if (selectedPlanIDs.length > LEGACY_PLAN_HANDOVER_MAX_SELECTED_PLANS ||
      selectedPlanIDs.some((id, index) => index > 0 && id === selectedPlanIDs[index - 1])) return null;
  const currentPlanIDs = new Set((plans || []).map((plan) => String(plan?.id || "")));
  if (selectedPlanIDs.some((id) => !currentPlanIDs.has(id))) return null;
  // False values are inert legacy noise, but malformed IDs or non-boolean
  // values must not be silently omitted from a future ownership handover.
  if (Object.entries(planPreferenceObject.plans).some(([id, enabled]) => !legacyPlanHandoverSafeID(id) || typeof enabled !== "boolean")) return null;

  const rawPlacePreferences = legacyPlanHandoverReadJSON(PLACE_WATCH_NOTIFICATION_PLACES_KEY);
  if (!rawPlacePreferences.known) return null;
  let selectedPlaceIDs = [];
  let placeSelectionMode = "default";
  // This flag is intentionally separate from the global permission/intention
  // above. A family can leave Nearcast notifications generally enabled while
  // pausing all saved-place watches; a future native owner must not turn that
  // paused setting into the implicit default-place selection.
  let placeNotificationsEnabled = false;
  if (rawPlacePreferences.value !== null) {
    const places = rawPlacePreferences.value;
    const permitted = new Set(["enabled", "selectedIds", "nativeDeletionWatermark", "updatedAt"]);
    if (!places || typeof places !== "object" || Array.isArray(places) ||
        Object.keys(places).some((key) => !permitted.has(key)) || typeof places.enabled !== "boolean" ||
        (Object.hasOwn(places, "nativeDeletionWatermark") && (!Number.isSafeInteger(places.nativeDeletionWatermark) || places.nativeDeletionWatermark < 0)) ||
        (Object.hasOwn(places, "updatedAt") && !legacyAgendaValidText(places.updatedAt, 40))) return null;
    placeNotificationsEnabled = places.enabled;
    if (Object.hasOwn(places, "selectedIds")) {
      if (!Array.isArray(places.selectedIds)) return null;
      selectedPlaceIDs = places.selectedIds.map((id) => String(id || "").trim());
      placeSelectionMode = "explicit";
      if (selectedPlaceIDs.length > LEGACY_PLAN_HANDOVER_MAX_SELECTED_PLACES ||
          selectedPlaceIDs.some((id) => !legacyPlanHandoverSafeID(id)) ||
          new Set(selectedPlaceIDs).size !== selectedPlaceIDs.length) return null;
    }
  }
  ...
  return {
    ...
    globalPreference,
    selectedPlanIDs,
    placeNotificationsEnabled,
    selectedPlaceIDs,
    placeSelectionMode
  };
}

function legacyPlanHandoverSnapshotFromStorage() {
  const agenda = legacyAgendaSnapshotFromStorage();
  if (!agenda) return null;
  const notificationIntent = legacyPlanHandoverNotificationIntent(agenda.plans);
  if (!notificationIntent) return null;
  return {
    version: LEGACY_PLAN_HANDOVER_EXPORT_VERSION,
    owner: "legacy",
    hydration: "ready",
    capturedAt: agenda.capturedAt,
    plans: agenda.plans,
    notificationIntent
  };
}

function publishLegacyPlanHandoverSnapshot() {
  if (!legacyAgendaMemoryHydrated || !planWatchInventoryIsKnown()) return false;
  const bridge = legacyAgendaIOSBridge();
  if (!bridge) return false;
  const handover = legacyPlanHandoverSnapshotFromStorage();
  if (!handover) return false;
  try {
    if (new TextEncoder().encode(JSON.stringify(handover)).byteLength > LEGACY_PLAN_HANDOVER_MAX_BYTES) return false;
    bridge.postMessage({ type: "plans.handover.snapshot", handover });
    return true;
  } catch {
    return false;
  }
