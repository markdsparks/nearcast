...
const STORM_IMPACT_ENABLED = true;
...
const MAP_ALERT_SOURCE_ID = "nearcast-official-alerts";
const MAP_ALERT_FILL_LAYER_ID = "nearcast-official-alert-fill";
const MAP_ALERT_LINE_LAYER_ID = "nearcast-official-alert-line";
...
function normalizeMapOpenIntent(value) {
  if (!value || typeof value !== "object" || value.currentTarget || value.target) return null;
  const sourceValue = String(value.source || value.layer || value.kind || "").trim().toLowerCase();
  const source = ["radar", "observed", "now"].includes(sourceValue)
    ? "radar"
    : ["forecast", "guidance", "future"].includes(sourceValue)
      ? "forecast"
      : ["alert", "warning", "watch"].includes(sourceValue)
        ? "alert"
        : "";
  ...
  const alertId = String(value.alertId || value.alertKey || "").trim();
  const event = String(value.event || value.eventLabel || value.label || value.title || "").trim();
  const placeId = String(value.placeId || place?.id || "").trim();
  const placeName = String(value.placeName || value.placeLabel || place?.name || "").trim();
  if (!source && timestamp == null && !alertId && !event && !placeId && !placeName) return null;
  return {
    ...
    longitude: Number.isFinite(longitude) ? normalizeMapLongitude(longitude) : null,
    setAt: Date.now()
  };
}

function setMapOpenIntent(options = {}) {
  const intent = normalizeMapOpenIntent(options);
  mapState.pendingOpenIntent = intent;
  return intent;
}

function takeMapOpenIntent(options = {}) {
  const explicit = normalizeMapOpenIntent(options);
...
function mapAlertGeometrySnapshot() {
  if (typeof window.nearcastAlertGeometrySnapshot !== "function") {
    return { state: "unknown", checkedAt: null, alerts: [] };
  }
  try {
    const snapshot = window.nearcastAlertGeometrySnapshot();
    return {
      state: String(snapshot?.state || "unknown"),
      checkedAt: Number(snapshot?.checkedAt) || null,
      alerts: Array.isArray(snapshot?.alerts) ? snapshot.alerts : []
    };
  } catch {
    return { state: "failed", checkedAt: null, alerts: [] };
  }
}

function mapAlertIntentMatches(alert, intent = mapState.openIntent) {
  const requested = String(intent?.alertId || "").trim();
  if (!requested || !alert) return false;
  const candidates = [alert.key, alert.id, alert.event, alert.headline]
    .map((value) => String(value || "").trim())
    .filter(Boolean);
  return candidates.some((value) => value === requested || value === requested.replace(/^id:/, ""));
}

function mapAlertToneRank(alert) {
  const tone = String(alert?.tone || "").toLowerCase();
  if (tone === "warning") return 0;
  if (tone === "watch") return 1;
  if (tone === "advisory") return 2;
  return 3;
}

function mapVisibleAlertRecords(snapshot = mapAlertGeometrySnapshot()) {
  return snapshot.alerts
    .filter((alert) => alert && typeof alert === "object")
    .sort((a, b) => {
      const selected = Number(mapAlertIntentMatches(b)) - Number(mapAlertIntentMatches(a));
      if (selected) return selected;
      const coverage = Number(b?.placeCoverage?.status === "inside") - Number(a?.placeCoverage?.status === "inside");
      if (coverage) return coverage;
      return mapAlertToneRank(a) - mapAlertToneRank(b);
    });
}

function mapAlertFeatureCollection(snapshot = mapAlertGeometrySnapshot()) {
  const features = mapVisibleAlertRecords(snapshot)
    .filter((alert) => ["Polygon", "MultiPolygon"].includes(alert?.geometry?.type))
    .slice(0, 6)
    .map((alert) => ({
      type: "Feature",
      id: String(alert.key || alert.id || alert.event || "official-alert"),
      properties: {
        key: String(alert.key || alert.id || ""),
        tone: String(alert.tone || "notice"),
        selected: mapAlertIntentMatches(alert) ? 1 : 0,
        coverage: String(alert?.placeCoverage?.status || "unknown")
      },
      geometry: {
...
function mapAlertColorExpression(alpha = 1) {
  const color = [
    "match", ["get", "tone"],
    "warning", `rgba(224, 67, 62, ${alpha})`,
    "watch", `rgba(236, 155, 44, ${alpha})`,
    "advisory", `rgba(236, 194, 72, ${alpha})`,
    `rgba(104, 166, 222, ${alpha})`
  ];
  return color;
}

function ensureMapLibreAlertLayers(record = mapLibreCurrentRecord()) {
  const map = record?.map;
  if (!map?.isStyleLoaded?.()) return;
  const collection = mapAlertFeatureCollection();
  const signature = JSON.stringify(collection);
  const source = map.getSource?.(MAP_ALERT_SOURCE_ID);
  if (!source) {
    map.addSource(MAP_ALERT_SOURCE_ID, { type: "geojson", data: collection });
    map.addLayer({
      id: MAP_ALERT_FILL_LAYER_ID,
      type: "fill",
      source: MAP_ALERT_SOURCE_ID,
      paint: {
        "fill-color": mapAlertColorExpression(0.2),
        "fill-opacity": ["case", ["==", ["get", "selected"], 1], 0.9, 0.62]
      }
    }, mapLibreWeatherBeforeLayer(map));
    map.addLayer({
      id: MAP_ALERT_LINE_LAYER_ID,
      type: "line",
      source: MAP_ALERT_SOURCE_ID,
      paint: {
        "line-color": mapAlertColorExpression(0.96),
        "line-width": ["case", ["==", ["get", "selected"], 1], 3.2, 2],
        "line-opacity": 0.94,
        "line-dasharray": [2, 1.25]
      }
    }, mapLibreWeatherBeforeLayer(map));
    record.alertGeometrySignature = signature;
  } else if (record.alertGeometrySignature !== signature) {
    source.setData?.(collection);
    record.alertGeometrySignature = signature;
  }
  const before = mapLibreWeatherBeforeLayer(map);
  if (before) {
    if (map.getLayer?.(MAP_ALERT_FILL_LAYER_ID)) map.moveLayer?.(MAP_ALERT_FILL_LAYER_ID, before);
    if (map.getLayer?.(MAP_ALERT_LINE_LAYER_ID)) map.moveLayer?.(MAP_ALERT_LINE_LAYER_ID, before);
  }
}

function syncMapAlertOverlays() {
  if (!mapState.immersive) return;
  if (mapRendererIsGl()) ensureMapLibreAlertLayers(mapLibreCurrentRecord());
  else renderStormImpactOverlay();
  syncMapIntentPresentation();
}

function mapIntentPlaceText(intent = mapState.openIntent) {
  if (state.activePlace) return placeLabel(state.activePlace);
  return String(intent?.placeName || "Selected place");
}

function mapAlertCoverageText(alert) {
  const status = String(alert?.placeCoverage?.status || "unknown");
  if (status === "inside") return "Your selected place is inside this alert";
  if (status === "outside") return "Your selected place is outside this alert";
  if (alert?.geometry) return "Official alert boundary shown";
  return "Official boundary is not available";
}

function mapIntentTimeText(intent = mapState.openIntent) {
  if (intent?.timestamp === null || intent?.timestamp === undefined || intent?.timestamp === "") return "";
  if (!Number.isFinite(Number(intent.timestamp))) return "";
...
function syncMapIntentPresentation() {
  ...
  if (!card) return;
  const kicker = document.getElementById("immMapContextKicker");
  const title = document.getElementById("immMapContextTitle");
  const meta = document.getElementById("immMapContextMeta");
  const surface = document.getElementById("immersiveMap");
  const snapshot = mapAlertGeometrySnapshot();
  const alerts = mapVisibleAlertRecords(snapshot);
  const selectedAlert = alerts.find((alert) => mapAlertIntentMatches(alert))
    || alerts.find((alert) => (
      alert?.placeCoverage?.status === "inside"
      && ["warning", "watch"].includes(String(alert?.tone || "").toLowerCase())
    ))
    || null;
  const intent = mapState.openIntent;

  if (selectedAlert) {
    card.hidden = false;
    surface?.classList.add("has-map-context");
    card.dataset.tone = String(selectedAlert.tone || "notice");
    if (kicker) kicker.textContent = "Official weather alert";
    if (title) title.textContent = selectedAlert.headline || selectedAlert.event || "Weather alert";
    if (meta) meta.textContent = `${mapAlertCoverageText(selectedAlert)} · ${mapIntentPlaceText(intent)}`;
    return;
  }
  ...
  if (intent?.source === "alert") {
    ...
    if (kicker) kicker.textContent = "Official weather alert";
    if (title) title.textContent = intent.event || "Weather alert";
    if (meta) {
      const boundaryState = snapshot.state === "checking" ? "Checking the official boundary" : "Official boundary is not available";
      meta.textContent = `${boundaryState} · ${mapIntentPlaceText(intent)}`;
    }
    return;
  }

  if (intent && (intent.event || intent.timestamp != null)) {
    card.hidden = false;
    surface?.classList.add("has-map-context");
  ...
  card.hidden = true;
  surface?.classList.remove("has-map-context");
  card.removeAttribute("data-tone");
}

function startMapTrustClock() {
  if (mapState.mapTrustTimer) clearInterval(mapState.mapTrustTimer);
  mapState.mapTrustTimer = setInterval(() => {
    if (!mapState.immersive) return;
...
function stopMapTrustClock() {
  if (!mapState.mapTrustTimer) return;
  clearInterval(mapState.mapTrustTimer);
  mapState.mapTrustTimer = 0;
}

function xweatherStormPreferenceEnabled() {
  const modeEnabled = typeof xweatherStormEnabled === "function" ? xweatherStormEnabled() : false;
  const activated = typeof window.nearcastXweatherStormActivated === "function"
    ? window.nearcastXweatherStormActivated()
    : false;
  return Boolean(modeEnabled && activated);
}
...
function xweatherStormCredentials() {
  if (typeof window.nearcastXweatherStormCredentials === "function") {
    const credentials = window.nearcastXweatherStormCredentials();
...
function xweatherStormConfigStatus() {
  if (typeof window.nearcastXweatherStormConfig !== "function") return { status: "unknown" };
...
function xweatherStormWeatherRelevance(record = mapLibreCurrentRecord()) {
  const snapshot = typeof window.nearcastStormScopeRelevance === "function"
    ? window.nearcastStormScopeRelevance()
    : { eligible: false, promote: false, reason: "weather-truth-unavailable", tone: "quiet" };
  const place = state.activePlace;
  const bounds = record?.map?.getBounds?.();
  ...
  if (
    ...
    return {
      ...
      eligible: false,
      promote: false,
      reason: "viewport-away-from-active-place",
      tone: "quiet"
    };
  }
  return snapshot;
...
function xweatherStormViewportContext(record = mapLibreCurrentRecord()) {
  ...
  const relevance = xweatherStormWeatherRelevance(record);
  return {
    contextKey,
    viewport,
    storm: {
      activeWeather: Boolean(relevance.eligible),
      activeWeatherReason: relevance.reason || "weather-truth-unavailable",
      promote: Boolean(relevance.promote),
      tone: relevance.tone || "quiet"
    }
  };
}
...
function xweatherStormUserMessage(error, fallback = `${XWEATHER_STORM_SHORT_NAME} unavailable`) {
  const name = String(error?.name || "").toLowerCase();
  const message = String(error?.message || "").trim();
  const lower = message.toLowerCase();
  if (name === "aborterror" || lower.includes("abort")) return `${XWEATHER_STORM_SHORT_NAME} timed out`;
  if (lower.includes("timed out") || lower.includes("taking too long")) return `${XWEATHER_STORM_SHORT_NAME} timed out`;
...
function xweatherStormGuard(record = mapLibreCurrentRecord()) {
  ...
  if (Number.isFinite(zoom) && zoom + 0.001 < xweatherStormClientMinZoom()) {
    return { ok: false, status: "below-min-zoom", message: `Zoom in to start ${XWEATHER_STORM_SHORT_NAME}`, context: requestContext };
  }
  if (!requestContext.storm.activeWeather) {
    return { ok: false, status: "no-active-weather", message: `${XWEATHER_STORM_SHORT_NAME} starts when radar is active`, context: requestContext };
  }
  const config = xweatherStormConfigStatus();
  if (config.status === "error" && xweatherStormConfigMatchesContext(config, requestContext)) {
    ...
    if (retryAt && Date.now() >= retryAt) {
      return { ok: false, status: "loading", message: `Checking ${XWEATHER_STORM_SHORT_NAME}`, context: requestContext };
    }
    return {
      ok: false,
      status: "error",
      message: config.message || `${XWEATHER_STORM_SHORT_NAME} unavailable`,
  ...
  if (!credentials.clientId || !credentials.clientSecret) {
    ...
    if (["needs-context", "activation-required", "below-min-zoom", "no-active-weather", "budget-paused", "provider-budget-paused", "paused", "error"].includes(config.status)) {
      return {
        ok: false,
        status: config.status,
        message: config.message || `${XWEATHER_STORM_SHORT_NAME} paused`,
        context: requestContext
      };
    }
    return { ok: false, status: "missing-keys", message: `${XWEATHER_STORM_SHORT_NAME} not configured`, context: requestContext };
  }
...
function xweatherStormStatusForChip() {
  ...
  if (status === "error") return { visible: true, ready: false, text: message || `${XWEATHER_STORM_SHORT_NAME} unavailable` };
  if ([
    "missing-keys",
    "activation-required",
    "budget",
...
function xweatherStormActivationControlState(record = mapLibreCurrentRecord()) {
  ...
  const activated = xweatherStormPreferenceEnabled();
  if (active) {
    return {
      visible: true,
      enabled: true,
      ...
      detail: `Leave ${XWEATHER_STORM_SHORT_NAME}`
    };
  }
  if (record?.xweatherStorm?.status === "error") {
    return {
      ...
      tone: "paused",
      action: "retry",
      label: `Retry ${XWEATHER_STORM_SHORT_NAME}`,
      detail: record.xweatherStorm.message || `${XWEATHER_STORM_SHORT_NAME} failed`
    };
  }
  if (record?.xweatherStorm?.status === "loading") {
    if (xweatherStormRecordLoadingStale(record)) {
      return {
        ...
        action: "retry",
        label: `Retry ${XWEATHER_STORM_SHORT_NAME}`,
        detail: `${XWEATHER_STORM_SHORT_NAME} timed out`
      };
    }
    return {
      visible: true,
      ...
      tone: "loading",
      action: "cancel",
      label: `Cancel ${XWEATHER_STORM_SHORT_NAME}`,
  ...
  if (
    config.status === "below-min-zoom"
    && Number.isFinite(zoom)
    && zoom + 0.001 < xweatherStormClientMinZoom()
  ) {
    return {
      visible: true,
      enabled: false,
      ...
      action: "none",
      label: "Zoom in",
      detail: `${XWEATHER_STORM_SHORT_NAME} at street radar`
    };
  }
  if (config.status === "no-active-weather" && !context.storm.activeWeather) {
    return {
      visible: true,
      enabled: false,
      tone: "standby",
  ...
  if (!context.storm.activeWeather) {
    return {
      visible: true,
      enabled: false,
      tone: "standby",
      action: "none",
      label: "No active weather nearby",
      detail: "Nearcast Radar is selected"
    };
  }
  if (Number.isFinite(zoom) && zoom + 0.001 < xweatherStormClientMinZoom()) {
    return {
  ...
  return {
    ...
    tone: "ready",
    action: "activate",
    label: XWEATHER_STORM_SHORT_NAME,
    detail: "Use enhanced storm map"
  };
}

function radarViewState(record = mapLibreCurrentRecord()) {
  const storm = xweatherStormActivationControlState(record);
  const context = xweatherStormViewportContext(record);
  const config = xweatherStormConfigStatus();
  const stormActive = xweatherStormActive(record);
  const highDetail = rawMapHighDetailVisible(mapState.frames[mapState.frameIndex], record);
  const zoom = Number(context.viewport?.zoom);
  const belowMinZoom = Number.isFinite(zoom) && zoom + 0.001 < xweatherStormClientMinZoom();
  const budgetPaused = xweatherStormBudgetPaused(config);

  let stormStatus = "Ready to check";
  let stormDetail = "Starts only when you ask.";
  let primaryAction = "";
  let primaryLabel = "";
  let stormState = "standby";

  if (stormActive) {
    stormStatus = "Active";
    stormDetail = "Motion radar is on.";
    primaryAction = "deactivate";
    primaryLabel = "Use Nearcast Radar";
    stormState = "active";
  } else if (storm.action === "cancel") {
    stormStatus = "Starting";
    stormDetail = "Checking protected beta access.";
    primaryAction = "cancel";
    primaryLabel = "Cancel";
    stormState = "loading";
  } else if (!context.storm.activeWeather) {
    stormStatus = "No active weather nearby";
    stormDetail = "Nearcast Radar is the useful view right now.";
  } else if (belowMinZoom) {
    stormStatus = "Zoom in to use";
    stormDetail = "StormScope starts at a closer radar scale.";
  } else if (budgetPaused) {
    stormStatus = "Beta paused";
    stormDetail = "The provider budget is protected.";
    if (storm.enabled) {
      primaryAction = "retry";
      primaryLabel = "Check again";
    }
    stormState = "paused";
  } else if (storm.action === "retry" || record?.xweatherStorm?.status === "error") {
    stormStatus = "Unavailable";
    stormDetail = record?.xweatherStorm?.message || "Try checking access again.";
    primaryAction = "retry";
    primaryLabel = "Try again";
    stormState = "paused";
  } else if (storm.action === "explain" || !storm.enabled) {
    stormStatus = "Beta unavailable";
    stormDetail = storm.detail || "Protected provider access is not available right now.";
    if (storm.enabled) {
      primaryAction = "retry";
      primaryLabel = "Check again";
    }
    ...
    stormStatus = "Useful now";
    stormDetail = "A storm signal is active near this place.";
  ...
  return {
    storm,
    context,
    controlTone: stormActive ? "active" : stormState,
    nearcastStatus: highDetail ? "High detail active" : stormActive ? "Ready when you return" : "Automatic",
    nearcastDetail: highDetail
      ? "Nearcast is drawing the clearest included source available."
      : "Nearcast chooses the clearest available source.",
    nearcastActive: !stormActive,
    stormStatus,
    stormDetail,
    stormState,
    stormActive,
...
function syncXweatherStormActivationControl(record = mapLibreCurrentRecord()) {
  ...
  const state = radarViewState(record);
  const entryVisible = Boolean(mapState.immersive && !mapSatelliteEnabled());
  entry.hidden = !entryVisible;
  entry.disabled = false;
  entry.dataset.tone = state.controlTone || "standby";
  entry.dataset.action = "open";
  entry.setAttribute("aria-pressed", String(state.stormActive));
  entry.setAttribute("aria-label", `Radar view. ${state.stormActive ? "StormScope active" : `${state.nearcastStatus}. ${state.stormStatus}`}`);
  entry.setAttribute("title", "Radar view");
  if (stormViewSheetOpen()) renderXweatherStormSheet();
  syncXweatherLightningToggleControl(record);
}

window.nearcastSyncRadarView = () => {
  syncXweatherStormActivationControl(mapLibreCurrentRecord());
  if (mapState.immersive) renderImmersiveTimelineReadout();
    ...
    Promise.resolve(window.nearcastLoadXweatherStormConfig({
      ...
      .then((config) => {
        if (
          ...
          !mapState.immersive ||
          !xweatherStormPreferenceEnabled()
        ) return;
...
function endXweatherStormSession(record = mapLibreCurrentRecord(), reason = "off") {
  stopXweatherStormLayer(record, reason);
  if (typeof window.nearcastDeactivateXweatherStorm === "function") {
    window.nearcastDeactivateXweatherStorm();
  }
  if (reason === "off" && mapState.immersive && !mapSatelliteEnabled()) {
...
function stormViewSheetElements() {
  const nearcastStatus = document.querySelector('[data-radar-view-status="nearcast"]');
  const stormscopeStatus = document.querySelector('[data-radar-view-status="stormscope"]');
  return {
    ...
    nearcastOption: document.querySelector('[data-radar-view-option="nearcast"]'),
    nearcastStatus,
    nearcastStatusTitle: nearcastStatus?.querySelector("strong") || null,
    nearcastStatusDetail: nearcastStatus?.querySelector("span:last-child") || null,
    stormscopeOption: document.querySelector('[data-radar-view-option="stormscope"]'),
    stormscopeStatus,
    stormscopeStatusTitle: stormscopeStatus?.querySelector("strong") || null,
    stormscopeStatusDetail: stormscopeStatus?.querySelector("span:last-child") || null,
...
function xweatherStormSheetCopy() {
  const view = radarViewState(mapLibreCurrentRecord());
  let cost = `${XWEATHER_STORM_SHORT_NAME} uses paid provider data. Beta access may pause to protect the test budget.`;
  if (view.stormActive) {
    cost = "StormScope is active. Nearcast Radar remains ready whenever you switch back.";
  } else if (view.stormState === "paused") {
    cost = "Nearcast Radar stays available while protected StormScope beta access is paused.";
  } else if (!view.context.storm.activeWeather) {
    cost = "No active storm signal is nearby, so Nearcast Radar is the useful view right now.";
  } else if (view.stormStatus === "Zoom in to use") {
    cost = "Zoom in for the closer storm scale required by StormScope.";
  }
  return {
    mode: view.stormState,
    kicker: "Radar view",
    title: "Choose the right level of detail",
    summary: "Nearcast adapts automatically. Check StormScope when you need a closer storm read.",
    nearcastStatus: view.nearcastStatus,
    nearcastDetail: view.nearcastDetail,
    nearcastActive: view.nearcastActive,
    stormStatus: view.stormStatus,
    stormDetail: view.stormDetail,
    stormState: view.stormState,
    stormActive: view.stormActive,
    cost,
...
function renderXweatherStormSheet() {
  const elements = stormViewSheetElements();
  const copy = xweatherStormSheetCopy();
  ...
  if (elements.nearcastOption) {
    elements.nearcastOption.dataset.state = copy.nearcastActive ? "active" : "standby";
    elements.nearcastOption.setAttribute("aria-current", copy.nearcastActive ? "true" : "false");
  }
  if (elements.nearcastStatus) elements.nearcastStatus.dataset.state = copy.nearcastActive ? "active" : "standby";
  if (elements.nearcastStatusTitle) elements.nearcastStatusTitle.textContent = copy.nearcastStatus;
  if (elements.nearcastStatusDetail) elements.nearcastStatusDetail.textContent = copy.nearcastDetail;
  if (elements.stormscopeOption) {
    elements.stormscopeOption.dataset.state = copy.stormState || "standby";
    elements.stormscopeOption.setAttribute("aria-current", copy.stormActive ? "true" : "false");
  }
  if (elements.stormscopeStatus) elements.stormscopeStatus.dataset.state = copy.stormState || "standby";
  if (elements.stormscopeStatusTitle) elements.stormscopeStatusTitle.textContent = copy.stormStatus;
  if (elements.stormscopeStatusDetail) elements.stormscopeStatusDetail.textContent = copy.stormDetail;
...
function activateXweatherStormFromSheet() {
  ...
  if (["deactivate", "cancel"].includes(action)) {
    endXweatherStormSession(record, "off");
    renderMapLibreOverlays({ forceRadar: true });
    return;
  }
  if (["activate", "retry"].includes(action)) startXweatherStormActivation(record);
}

function handleXweatherStormEntry() {
  openXweatherStormSheet({ force: true });
}
...
function xweatherStormTimelineWindow(nowMs = Date.now()) {
  return {
    nowMs,
    startMs: nowMs - XWEATHER_STORM_TIMELINE_PAST_MS,
    endMs: nowMs + XWEATHER_STORM_TIMELINE_FUTURE_MS
  };
}

function configureXweatherStormTimeline(controller, nowMs = Date.now()) {
  const timeline = controller?.timeline;
  const range = xweatherStormTimelineWindow(nowMs);
  if (!timeline) return { ...range, timeline: null };
  const startDate = new Date(range.startMs);
  const endDate = new Date(range.endMs);
  const nowDate = new Date(range.nowMs);

  try { timeline.startDate = startDate; } catch {}
  try { timeline.endDate = endDate; } catch {}
  try { timeline.duration = XWEATHER_STORM_TIMELINE_LOOP_SECONDS; } catch {}
  try { timeline.goToDate(nowDate); } catch {
    try { timeline.goTo(0.5); } catch {}
  }
  try { timeline.pause?.(); } catch {}
  return { ...range, timeline };
}

function xweatherStormTimelineSnapshot(record = mapLibreCurrentRecord()) {
  const storm = record?.xweatherStorm;
  const timeline = storm?.timeline || storm?.controller?.timeline;
  const info = timeline?.info || {};
  const startMs = dateMs(info.startDate) ?? dateMs(timeline?.startDate) ?? storm?.timelineStartMs;
  const endMs = dateMs(info.endDate) ?? dateMs(timeline?.endDate) ?? storm?.timelineEndMs;
  const nowMs = Number.isFinite(Number(storm?.timelineNowMs)) ? Number(storm.timelineNowMs) : Date.now();
  const currentMs = dateMs(info.currentDate) ?? dateMs(timeline?.currentDate) ?? nowMs;
  const span = Math.max(1, (endMs ?? nowMs) - (startMs ?? nowMs));
  const position = Number.isFinite(Number(timeline?.position))
    ? Number(timeline.position)
    : (currentMs - (startMs ?? currentMs)) / span;
  return {
    timeline,
    startMs: startMs ?? nowMs - XWEATHER_STORM_TIMELINE_PAST_MS,
    endMs: endMs ?? nowMs + XWEATHER_STORM_TIMELINE_FUTURE_MS,
    nowMs,
    currentMs,
    position: clamp(position, 0, 1),
    nowPosition: clamp((nowMs - (startMs ?? nowMs)) / span, 0, 1),
    playing: Boolean(timeline?.isAnimating || info.isActive && timeline?.state === "playing")
  };
}
...
function syncXweatherStormTimelineHud(record = mapLibreCurrentRecord(), options = {}) {
  ...
  if (slider) {
    slider.min = "0";
    slider.max = String(XWEATHER_STORM_TIMELINE_STEPS);
    slider.value = String(Math.round(snapshot.position * XWEATHER_STORM_TIMELINE_STEPS));
  ...
  if (marker) {
    marker.hidden = !mapState.immersive;
    marker.style.left = `${snapshot.nowPosition * 100}%`;
  }
...
function renderXweatherStormTimelineBubble(snapshot = xweatherStormTimelineSnapshot(), options = {}) {
  const bubble = document.getElementById("immTimeBubble");
  const slider = els.frameSlider;
  if (!bubble || !slider || !mapState.immersive) return;
  const copy = xweatherStormTimelineCopy(snapshot);
...
function scheduleXweatherStormTimelineHudSync(record = mapLibreCurrentRecord(), options = {}) {
  if (!record?.xweatherStorm || record.xweatherStorm.timelineSyncRaf) return;
  record.xweatherStorm.timelineSyncRaf = requestAnimationFrame(() => {
    if (!record.xweatherStorm) return;
    record.xweatherStorm.timelineSyncRaf = 0;
    syncXweatherStormTimelineHud(record, options);
  });
}
...
function clearXweatherStormTimelineBindings(record) {
  const storm = record?.xweatherStorm;
  if (!storm) return;
  (storm.timelineCleanupFns || []).forEach((cleanup) => {
    try { cleanup(); } catch {}
  });
  storm.timelineCleanupFns = [];
  if (storm.timelineSyncRaf) {
    cancelAnimationFrame(storm.timelineSyncRaf);
    storm.timelineSyncRaf = 0;
  }
...
function addXweatherTimelineListener(timeline, eventName, handler, cleanups) {
  try {
    if (typeof timeline?.on === "function") {
      timeline.on(eventName, handler);
      cleanups.push(() => timeline.off?.(eventName, handler));
      return true;
    }
    if (typeof timeline?.addEventListener === "function") {
      timeline.addEventListener(eventName, handler);
      cleanups.push(() => timeline.removeEventListener?.(eventName, handler));
      return true;
    }
  } catch {}
  return false;
...
function bindXweatherStormTimeline(record) {
  const storm = record?.xweatherStorm;
  const timeline = storm?.timeline || storm?.controller?.timeline;
  if (!storm || !timeline) return;
  clearXweatherStormTimelineBindings(record);
  storm.timeline = timeline;
  storm.timelineCleanupFns = [];
  const sync = () => scheduleXweatherStormTimelineHudSync(record);
  [
    "advance",
    "range:change",
    "change",
    ...
    "stop",
    "complete"
  ].forEach((eventName) => addXweatherTimelineListener(timeline, eventName, sync, storm.timelineCleanupFns));
  setPlaybackButtonState(els.playRadar, mapState.playing);
  syncXweatherStormTimelineHud(record);
}

function setXweatherStormTimelinePlaying(record = mapLibreCurrentRecord(), playing = false) {
  if (!xweatherStormActive(record)) return false;
  const storm = record.xweatherStorm;
  const timeline = storm.timeline || storm.controller?.timeline;
  ...
  scheduleXweatherStormTimelineHudSync(record);
  return true;
}

function scrubXweatherStormTimeline(position) {
  const record = mapLibreCurrentRecord();
  if (!xweatherStormActive(record)) return false;
  if (mapState.playing) {
    mapState.userPausedRadar = true;
    setXweatherStormTimelinePlaying(record, false);
  }
...
function toggleXweatherStormTimelinePlayback(record = mapLibreCurrentRecord()) {
  if (!xweatherStormActive(record)) return false;
  return setXweatherStormTimelinePlaying(record, !mapState.playing);
}

function xweatherMapsglEnumValue(group, key, fallback) {
...
function addXweatherStormWeatherLayer(controller, code, sdk) {
  const options = xweatherStormLayerOptions(code, sdk);
  if (options) {
    try {
      controller.addWeatherLayer(code, options);
      return true;
    } catch (error) {
...
async function startXweatherStormLayer(record) {
  ...
  record.xweatherStorm.loadingPromise = (async () => {
    ...
    const timelineWindow = xweatherStormTimelineWindow();
    const controller = new sdk.MaplibreMapController(record.map, {
      account,
      animation: {
        start: new Date(timelineWindow.startMs),
        end: new Date(timelineWindow.endMs),
        autoplay: false,
        repeat: true,
        duration: XWEATHER_STORM_TIMELINE_LOOP_SECONDS,
        alwaysPlayFromBeginning: false,
        pauseWhileLoading: true,
        resumeOnMoveEnd: true,
        preloadData: true
      }
    });
    const slowReadyTimer = setTimeout(() => {
      if (mapLibreCurrentRecord() === record && record.xweatherStorm?.loadingPromise) {
        setXweatherStormStatus(record, "loading", `Still loading ${XWEATHER_STORM_SHORT_NAME}`);
      }
    ...
    if (!controllerReady) {
      try { controller.dispose?.(); } catch {}
      try { controller.destroy?.(); } catch {}
      throw new Error(`${XWEATHER_STORM_SHORT_NAME} is taking too long`);
    }
    const timelineState = configureXweatherStormTimeline(controller, timelineWindow.nowMs);
    const addedLayers = [];
    layerCodes.forEach((code) => {
      try {
        addXweatherStormWeatherLayer(controller, code, sdk);
    ...
    disposeRawMapEnhancement("stormscope-active");
    setMapLibreStormLabelEmphasis(record, true);
    stopRadarPlayback({ renderStatic: false });
    record.xweatherStorm = {
      status: "active",
      controller,
      timeline: timelineState.timeline || controller.timeline || null,
      timelineStartMs: timelineState.startMs,
      timelineEndMs: timelineState.endMs,
      timelineNowMs: timelineState.nowMs,
      timelineCleanupFns: [],
      timelineSyncRaf: 0,
      timelineLoopRaf: 0,
      ...
      message: XWEATHER_STORM_SHORT_NAME
    };
    ...
    console.warn("[Nearcast] Xweather StormScope failed", error);
...
function restoreStandardRadarTimelineAfterStorm(record, options = {}) {
  if (mapState.timer) {
  ...
  if (options.render && mapState.frames.length) showFrame(nextIndex);
  else updateTimelineEraVisuals();
}
...
function stopXweatherStormLayer(record, reason = "off") {
  ...
  if (reason === "error") {
    record.xweatherStorm = { status: "error", message: storm.message || `${XWEATHER_STORM_SHORT_NAME} unavailable`, updatedAt: Date.now() };
  } else {
    record.xweatherStorm = null;
    if (!["destroy", "exit"].includes(reason) && record.loaded) {
      restoreStandardRadarTimelineAfterStorm(record, { render: true });
    }
  }
  syncGeneratedRadarStatusChip();
  renderMapLegend();
...
function xweatherStormDiagnosticsSnapshot(record = mapLibreCurrentRecord()) {
  ...
  const timeline = storm?.status === "active" ? xweatherStormTimelineSnapshot(record) : null;
  return {
    ...
    timeline: timeline ? {
      ...
      current: new Date(timeline.currentMs).toISOString(),
      start: new Date(timeline.startMs).toISOString(),
      end: new Date(timeline.endMs).toISOString(),
      playing: Boolean(mapState.playing)
    } : null,
...
function openInlineMapPreviewIntent() {
  let intent = null;
  if (typeof window.nearcastInlineMapIntent === "function") {
    try { intent = window.nearcastInlineMapIntent(); } catch {}
  }
  return enterImmersiveMap(intent || {});
}
...
function renderMapLibreOverlays(options = {}) {
  ...
  if (mapState.immersive) ensureMapLibreAlertLayers(record);
...
function mapAerialSupported(place = state.activePlace) {
  ...
  if (latitude >= 13 && latitude <= 15 && longitude >= 143 && longitude <= 146) return true;
  if (latitude >= 17 && latitude <= 23 && longitude >= -161 && longitude <= -65) return true;
  if (latitude >= 51 && latitude <= 72 && longitude >= -170 && longitude <= -129) return true;
  return latitude >= 20 && latitude <= 55 && longitude >= -130 && longitude <= -60;
}

function mapAerialEnabled() {
...
function mapLibreStyle() {
  ...
  return {
    ...
    sources: {
      ...
      "nearcast-aerial": {
        type: "raster",
        tiles: [USGS_AERIAL_TILE_TEMPLATE],
        tileSize: 256,
        minzoom: MAP_MIN_ZOOM,
    ...
    layers: [
      ...
      {
        id: MAPLIBRE_AERIAL_LAYER_ID,
        type: "raster",
        source: "nearcast-aerial",
        layout: {
...
function rawMapHighDetailVisible(
  frame = mapState.frames[mapState.frameIndex],
  record = mapLibreCurrentRecord()
) {
  return Boolean(
    mapState.immersive
    && !mapSatelliteEnabled()
    && !xweatherStormActive(record)
    && (
      rawMapFrameLayerVisible(frame, record)
      || mapLibreRawFrameHandoffSuppressesFallback(record)
    )
  );
}
...
function xweatherStormDiagnosticLine(storm) {
  ...
  const timeline = storm?.timeline ? ` · t ${Math.round((storm.timeline.position || 0) * 100)}%` : "";
  const detail = storm?.message && storm.status !== "active" ? ` · ${storm.message}` : "";
  return `StormScope ${active}${layers ? ` · ${layers}` : ""}${timeline}${usage}${detail}`;
}
...
function syncMapZoomDebugReadout() {
  ...
  if (!DEBUG_SETTINGS_ENABLED) {
    els.weatherMap?.querySelector(":scope > .map-zoom-debug")?.remove();
    return;
  }
...
function syncGeneratedRadarStatusChip() {
  ...
  // Source quality already appears in the Time Lens receipt. A second
  // permanent “High detail” badge competes with the map without helping a
  // family answer what the weather is doing. Keep this chip for short-lived
  // StormScope activation/closeout state only.
  chip.hidden = true;
  chip.classList.remove("is-ready", "is-high-detail");
  chip.textContent = "";
}
...
async function loadMapFrames(force = false, options = {}) {
  ...
  if (["radar", "precip"].includes(timelineKind)) mapState.radarLoadState = "checking";
  ...
  try {
    ...
    if (["radar", "precip"].includes(timelineKind)) {
      mapState.radarLoadState = timeline.radarState || (timeline.frames.some((frame) => activeMapSource(frame) === "radar") ? "available" : "unavailable");
    }
...
async function fetchMapTimeline(timelineKind) {
  ...
  if (timelineKind === "forecast") {
    ...
    return {
      ...
      emptyLabel: "Forecast map unavailable here"
    };
  }
  ...
  return {
    ...
    radarState: frames.length ? "available" : "unavailable",
    forecastUnavailable: false,
    emptyLabel: "Live radar unavailable"
  };
}
...
async function fetchPrecipTimelineFrames() {
  ...
  return {
    ...timeline,
    radarState: radarFrames.length ? "available" : radarResult.status === "rejected" ? "unavailable" : "unavailable",
    forecastUnavailable,
...
function renderMapCredit(frame = mapState.frames[mapState.frameIndex]) {
  ...
  const baseCredit = mapAerialEnabled()
    ...
    : 'Map <a href="https://carto.com/basemaps/" target="_blank" rel="noreferrer">CARTO</a> © <a href="https://www.openstreetmap.org/copyright" target="_blank" rel="noreferrer">OSM</a>';
  const html = `${escapeHtml(credit)} · ${baseCredit}`;
  document.querySelectorAll("#mapCredit, #immCredit").forEach((node) => {
    node.innerHTML = html;
  });
  document.getElementById("immCreditToggle")?.setAttribute(
...
function mapRadarSignal() {
  if (typeof currentRadarPrecipSignal !== "function") return null;
  try { return currentRadarPrecipSignal(state.activePlace); }
  catch { return null; }
}

function mapRadarAgeMinutes(timestamp, now = Date.now()) {
  const value = Number(timestamp);
  if (!Number.isFinite(value)) return null;
  return Math.max(0, Math.round((now - value) / 60000));
}

function mapRadarTrustState(frame = mapState.frames[mapState.frameIndex], now = Date.now()) {
  const source = activeMapSource(frame);
  if (source === "forecast") {
    const unavailable = mapState.radarLoadState === "unavailable"
      || !mapState.frames.some((candidate) => activeMapSource(candidate) === "radar");
    return unavailable
      ? { state: "unavailable", label: "Live radar unavailable · Forecast guidance", ageMin: null }
      : { state: "forecast", label: "Forecast guidance", ageMin: null };
  }
  if (!frame) {
    const checking = mapState.radarLoadState === "checking";
    return checking
      ? { state: "checking", label: "Checking live radar", ageMin: null }
      : { state: "unavailable", label: "Live radar unavailable", ageMin: null };
  }

  const frameIndex = mapState.frames.indexOf(frame);
  const isLatest = Boolean(frame.isNow || frameIndex === mapState.nowIndex);
  const timestamp = Number(frame.observedTimestamp)
    || Date.parse(frame.rawMapValidTime || "")
    || rawMapTimelineTimestamp(frame);
  const ageMin = mapRadarAgeMinutes(timestamp, now);
  if (!isLatest) {
    return {
      state: "historical",
      label: ageMin == null ? "Observed radar" : `Observed radar · ${ageMin} min ago`,
      ageMin
    };
  }
  if (ageMin == null || now - timestamp > MAP_RADAR_DELAYED_MAX_AGE_MS) {
    return { state: "unavailable", label: "Live radar unavailable · Last image is too old", ageMin };
  }
  if (now - timestamp > MAP_RADAR_CURRENT_MAX_AGE_MS) {
    return { state: "delayed", label: `Radar delayed · Last image ${ageMin} min ago`, ageMin };
  }
  const signal = mapRadarSignal();
  const signalIsFresh = signal && now - Number(signal.checkedAt || 0) <= MAP_RADAR_CURRENT_MAX_AGE_MS;
  if (signalIsFresh && signal.phase === "clear") {
    return { state: "clear", label: `Radar clear here · Updated ${ageMin || "just"}${ageMin ? " min ago" : " now"}`, ageMin };
  }
  return { state: "current", label: `Live radar · Updated ${ageMin || "just"}${ageMin ? " min ago" : " now"}`, ageMin };
}

function timelineReadoutCopy(frame = mapState.frames[mapState.frameIndex]) {
  if (!frame) {
    const trust = mapRadarTrustState(null);
    return {
      source: "radar",
  ...
  const trust = mapRadarTrustState(frame);
...
function renderImmersiveTimelineReadout(copy = timelineReadoutCopy()) {
  ...
  root.dataset.trustState = copy.trustState || (copy.source === "forecast" ? "forecast" : "current");
...
function setPlaybackButtonState(button = els.playRadar, playing = mapState.playing) {
  if (!button) return;
  const noun = xweatherStormActive(mapLibreCurrentRecord())
    ? "storm motion"
    : mapState.timelineKind === "precip"
...
function startRadarPlayback(options = {}) {
  if (xweatherStormActive(mapLibreCurrentRecord())) {
    setXweatherStormTimelinePlaying(mapLibreCurrentRecord(), true);
    return;
  }
...
function toggleRadarPlayback() {
  if (toggleXweatherStormTimelinePlayback(mapLibreCurrentRecord())) return;
  if (!mapState.frames.length) return;
  if (mapState.playing) {
    mapState.userPausedRadar = true;
    stopRadarPlayback();
...
function stopRadarPlayback(options = {}) {
  if (xweatherStormActive(mapLibreCurrentRecord())) {
    setXweatherStormTimelinePlaying(mapLibreCurrentRecord(), false);
    return;
  }
...
function clearMapLayers(options = {}) {
  ...
  updateTimelineEraVisuals();
  if (stormActive) syncXweatherStormTimelineHud(stormRecord);
}
...
function renderMapLegend() {
  ...
  if (xweatherStormActive(mapLibreCurrentRecord())) {
    ...
    const note = [
      ...
      stormImpactFeatureEnabled() ? "Tap a storm to check whether it may reach your places" : "",
  ...
  const sourceNotes = [];
  if (mapState.immersive && stormImpactFeatureEnabled()) {
    sourceNotes.push("Tap a storm to check where it may go");
  }
...
function aerialTileUrl({ z, x, y }) {
  return USGS_AERIAL_TILE_TEMPLATE
    .replace("{z}", z)
    .replace("{x}", x)
    .replace("{y}", y);
}
...
function mapIntentLoadOptions(intent = mapState.openIntent) {
  const targetTimestamp = mapIntentTimestamp(intent?.timestamp);
  const targetSource = intent?.source === "forecast"
    || (intent?.source === "alert" && targetTimestamp != null && targetTimestamp > Date.now() + 5 * 60 * 1000)
    ? "forecast"
    : intent?.source === "radar" || intent?.source === "alert" ? "radar" : "";
  return {
    timelineKind: "precip",
    focusNow: targetTimestamp == null,
    targetTimestamp,
    targetSource
  };
}

async function ensureMapIntentPlace(intent) {
  const latitude = Number(intent?.latitude);
  const longitude = Number(intent?.longitude);
  if (!Number.isFinite(latitude) || !Number.isFinite(longitude) || typeof loadPlace !== "function") return;
  const activeLatitude = Number(state.activePlace?.latitude);
  const activeLongitude = Number(state.activePlace?.longitude);
  const sameId = Boolean(intent.placeId && state.activePlace?.id && intent.placeId === state.activePlace.id);
  const sameCoordinates = Number.isFinite(activeLatitude) && Number.isFinite(activeLongitude)
    && Math.abs(activeLatitude - latitude) < 0.0001
    && Math.abs(normalizeMapLongitude(activeLongitude) - longitude) < 0.0001;
  if (sameId || sameCoordinates) return;
  await loadPlace({
    id: intent.placeId || `map-intent:${latitude.toFixed(4)},${longitude.toFixed(4)}`,
    name: intent.placeName || "Selected place",
    latitude,
    longitude
  });
}
...
function exitImmersiveMap() {
  ...
  disposeRawMapEnhancement("immersive-exit");
  clearStormImpact();
  stopImmersiveLocationWatch();
  immersiveLocationState.following = false;
...
function setMapBaseMode(mode) {
  ...
  const wasSatellite = mapSatelliteEnabled();
  const enteringSatellite = nextMode === "satellite" && !wasSatellite;
  if (enteringSatellite) {
    closeXweatherStormSheet();
    endXweatherStormSession(mapLibreCurrentRecord(), "satellite");
    disposeRawMapEnhancement("satellite");
  }
  ...
  if (mapSatelliteEnabled()) resolveLatestSatellitePass();
    ...
    mapSatelliteState.requestSeq += 1;
    if (mapState.immersive && (wasSatellite || rawMapExperimentEnabled())) {
      scheduleRawMapEnhancement(mapState.timelineKind);
    }
  }
}
...
function mapAlertClassicPath(geometry, viewport) {
  ...
  return (polygons || []).map((rings) => (rings || []).map((ring) => {
    const points = (ring || [])
      .map((coordinate) => {
        ...
        if (!Number.isFinite(latitude) || !Number.isFinite(longitude)) return null;
        return mapScreenPointForLatLon(latitude, longitude, viewport);
      })
      .filter(Boolean);
...
function renderMapAlertGeometryHtml(viewport) {
  if (!mapState.immersive || mapRendererIsGl()) return "";
  const paths = mapVisibleAlertRecords()
    .filter((alert) => ["Polygon", "MultiPolygon"].includes(alert?.geometry?.type))
    .slice(0, 6)
    .map((alert) => {
      const path = mapAlertClassicPath(alert.geometry, viewport);
      if (!path) return "";
      const selected = mapAlertIntentMatches(alert) ? "true" : "false";
      const tone = String(alert.tone || "notice");
      return `<path class="map-alert-footprint" data-tone="${escapeHtml(tone)}" data-selected="${selected}" d="${path}"></path>`;
    })
    .join("");
  return paths
    ? `<svg class="map-alert-footprints" aria-hidden="true" viewBox="0 0 ${viewport.width.toFixed(1)} ${viewport.height.toFixed(1)}"><g fill-rule="evenodd">${paths}</g></svg>`
    : "";
}
...
function renderStormImpactOverlay(viewport = null) {
  const layer = document.getElementById("immersiveImpactLayer");
  if (!layer) return;
  const vp = viewport || (mapState.immersive && state.activePlace ? getMapViewport() : null);
  const alertHtml = vp ? renderMapAlertGeometryHtml(vp) : "";
  if (!stormImpactFeatureEnabled()) {
  ...
  layer.innerHTML = `${alertHtml}${coneHtml}${trailHtml}${pathHtml}${coreHtml}${targetHtml}`;
}
