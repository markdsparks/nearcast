...
const els = {
  ...
  forecastReceiptStatusMeta: document.querySelector("#forecastReceiptStatusMeta"),
  forecastReceiptBackdrop: document.querySelector("#forecastReceiptBackdrop"),
  forecastReceiptSheet: document.querySelector("#forecastReceiptSheet"),
  forecastReceiptClose: document.querySelector("#forecastReceiptClose"),
  forecastReceiptContext: document.querySelector("#forecastReceiptContext"),
  forecastReceiptOverview: document.querySelector("#forecastReceiptOverview"),
  forecastReceiptHeadline: document.querySelector("#forecastReceiptHeadline"),
  forecastReceiptOverviewMeta: document.querySelector("#forecastReceiptOverviewMeta"),
  forecastReceiptFreshness: document.querySelector("#forecastReceiptFreshness"),
  forecastReceiptFreshnessMeta: document.querySelector("#forecastReceiptFreshnessMeta"),
  forecastReceiptSource: document.querySelector("#forecastReceiptSource"),
  ...
  forecastReceiptUncertaintyTitle: document.querySelector("#forecastReceiptUncertaintyTitle"),
  forecastReceiptUncertaintyText: document.querySelector("#forecastReceiptUncertaintyText"),
  forecastReceiptFootnote: document.querySelector("#forecastReceiptFootnote"),
...
function dailyPrecipProfile(data, dayIndex, options = {}) {
  const daily = data?.daily || {};
  const h = data?.hourly || {};
  const dayStr = daily.time?.[dayIndex];
  const thresholds = precipNoticeThresholds(data);
  const scopedIndices = Array.isArray(options.indices) ? new Set(options.indices) : null;
  const scopedProfiles = scopedIndices
    ? [...scopedIndices].map((index) => hourlyPrecipProfile(data, index))
    : [];
  const dailyAmount = scopedIndices
    ? scopedProfiles.reduce((sum, profile) => sum + Number(profile.precip || 0), 0)
  ...
  if (h.time?.length && dayStr) {
    h.time.forEach((time, index) => {
      ...
      if (scopedIndices && !scopedIndices.has(index)) return;
  ...
  // A deterministic model amount without ensemble probability support is not
  // enough to make rain the day's identity. Keep it as a low-chance timing cue
  // until at least one relevant hour clears the feature threshold.
...
// NWS phrases are published as time ranges, while the app renders a sequence
// of local forecast hours. Match by overlap rather than an exact timestamp so
// a 3-hour NWS period can inform each of its hourly cards without moving the
// signal into a neighboring hour.
function nwsConvectivePeriodForHour(data = state.forecast, index = -1) {
  const evidence = nwsConvectiveEvidenceByForecast.get(data);
  const startMs = parseForecastTimestamp(data?.hourly?.time?.[index], data);
  if (!evidence?.periods?.length || !Number.isFinite(startMs)) return null;
  const nextMs = parseForecastTimestamp(data?.hourly?.time?.[index + 1], data);
  const endMs = Number.isFinite(nextMs) && nextMs > startMs
    ? nextMs
    : startMs + 60 * 60 * 1000;
  return evidence.periods.find((period) =>
    Number.isFinite(period.startMs) && Number.isFinite(period.endMs) &&
    period.startMs < endMs && period.endMs > startMs
  ) || null;
}
...
// Ahead of now, radar cannot tell us whether a storm will occur. We therefore
// require both the official NWS hourly wording and a supported model thunder
// hour before upgrading the icon/label to “likely.” NWS on its own remains a
// useful, clearly qualified thunder possibility rather than a hard storm call.
function convectiveEvidenceForFutureHour(data = state.forecast, index = -1, profile = hourlyPrecipProfile(data, index)) {
  const period = nwsConvectivePeriodForHour(data, index);
  if (!period) return null;
  const modelThunder = isThunderCode(profile?.rawCode) && Boolean(
    Number(profile?.pop || 0) >= PRECIP_FEATURE_POP || profile?.amountSupported
  );
  return {
    ...period,
    level: modelThunder ? "likely" : "possible",
    label: modelThunder ? "Thunderstorms likely" : "Thunderstorms possible",
    source: modelThunder ? "nws-hourly-model" : "nws-hourly",
    basis: modelThunder ? "official-forecast-plus-model" : "official-forecast"
  };
}

function nwsConvectiveWindowForIndices(data, indices = []) {
  const candidates = (indices || []).map((index) => {
    const profile = hourlyPrecipProfile(data, index);
    const convective = convectiveEvidenceForFutureHour(data, index, profile);
    if (!convective) return null;
    return { index, profile, convective };
  }).filter(Boolean);
  if (!candidates.length) return null;

  // The first affected hour is the most useful planning cue. A consensus hour
  // wins when two signals begin together, but never shifts the timing later.
  const firstIndex = Math.min(...candidates.map((candidate) => candidate.index));
  const firstCandidates = candidates.filter((candidate) => candidate.index === firstIndex);
  const first = firstCandidates.find((candidate) => candidate.convective.level === "likely") || firstCandidates[0];
  const contiguous = candidates
    .filter((candidate) => candidate.index >= firstIndex)
    .sort((a, b) => a.index - b.index);
  let endIndex = firstIndex;
  for (const candidate of contiguous) {
    if (candidate.index > endIndex + 1) break;
  ...
  return {
    ...
    endIndex,
    startTime: data?.hourly?.time?.[firstIndex] || "",
    endTime: data?.hourly?.time?.[endIndex + 1] || ""
  };
}

function nwsConvectiveTimingPhrase(data, window, relevant = []) {
  if (!window) return "";
  const prefix = window.convective?.level === "likely" ? "Storms likely" : "Thunderstorms possible";
  const firstRelevant = relevant[0];
  const lastRelevant = relevant.at(-1);
  const startsAtBeginning = Number.isInteger(firstRelevant) && window.startIndex <= firstRelevant + 1;
  const runsToEnd = Number.isInteger(lastRelevant) && window.endIndex >= lastRelevant - 1;
  if (startsAtBeginning && !runsToEnd && window.endTime) return `${prefix} ending near ${formatTime(window.endTime)}`;
  if (!startsAtBeginning && window.startTime) return `${prefix} after ${formatTime(window.startTime)}`;
  return runsToEnd ? `${prefix} much of the day` : prefix;
}
...
function nowPrecipSignal(data = state.forecast) {
  ...
  const consider = (amount, intervalSeconds, code, chance = 0, isSnow = false, source = "modeled-current") => {
  ...
  // support "rain now," but only radar is described as observed or measured.
  if (current.basis === "modeled-current") {
    consider(current.precipitation, current.interval || 900, current.weather_code, 0, false, "modeled-current");
  }
  ...
  if (nowcast) {
    ...
    if (nowcast.wet?.[0]) {
      ...
      consider(slot?.precip, 15 * 60, nowcastBaseCode, slot?.prob, nowcast.isSnow, "modeled-15-minute");
...
function radarSignalForForecastData(data = state.forecast) {
  // Radar is sampled for the place currently on screen. Forecast objects used
  // for saved plans, remote AI questions, and background watch evaluations must
  // never borrow that observation.
  return data === state.forecast ? currentRadarPrecipSignal(state.activePlace) : null;
}

function applyRadarPrecipSignal(signal, data = state.forecast, radar = radarSignalForForecastData(data)) {
...
function buildPrecipTruth(data, nowPrecip, context = {}) {
  ...
  if (nowPrecip?.isWetNow) {
    ...
    const radarObserved = nowPrecip.source === "radar-current" || nowPrecip.radar?.phase === "active";
    const modeledDetail = nowPrecip.source === "modeled-15-minute"
      ? `${label} is indicated by the latest 15-minute forecast.`
      : `${label} is indicated by the latest current-conditions model.`;
    return {
      phase: "active",
      isWetNow: true,
      isImminent: false,
      ...
      detail: radarObserved ? (nowPrecip.detail || `${label} is observed on radar now.`) : modeledDetail,
      chance,
      startsInMin: 0,
      nowcast,
      source: radarObserved ? "radar-current" : (nowPrecip.source || "modeled-current"),
      basis: radarObserved ? "observed" : "forecast",
      confidence: radarObserved ? "observed" : "forecast"
    };
  }
...
function weatherTruthReceipt(display, nowPrecip, data = state.forecast, precipTruth = display?.precipTruth) {
  ...
  if (precipTruth?.phase === "active" || nowPrecip?.isWetNow) {
    ...
    const source = precipTruth?.source === "radar-current"
      ? "radar"
      : precipTruth?.source === "modeled-15-minute" ? "15-minute forecast" : "current forecast";
    const lowPopNote = Number.isFinite(hourlyPop) && hourlyPop < PRECIP_FEATURE_POP
      ? precipTruth?.source === "radar-current"
        ? ` Hourly rain chance is only ${hourlyPop}%, so radar observation is taking priority.`
        : ` Hourly rain chance is only ${hourlyPop}%, so the newer near-term forecast is taking priority.`
      : "";
    const basisCopy = precipTruth?.source === "radar-current"
      ? `${activeLabel} is observed over this place now.`
      : `${activeLabel} is indicated now by the latest near-term forecast.`;
    return {
      short: `${activeLabel} now · ${source}`,
      detail: `${basisCopy}${lowPopNote}`,
      source: precipTruth?.source || "modeled-current",
...
function buildWeatherTruth(data = state.forecast) {
  ...
  const currentIndex = currentHourlyIndex(data);
  const hourly = data?.hourly || {};
  const radar = radarSignalForForecastData(data);
  const nowPrecip = applyRadarPrecipSignal(nowPrecipSignal(data), data, radar);
  const radarSignalKey = radarPrecipSignalKey(radar);
  let display;
...
function forecastConditionFamily(code) {
  if (isThunderCode(code)) return "storm";
  if (isSnowCode(code)) return "snow";
  if (isPrecipCode(code)) return "rain";
...
function forecastStoryCondition(code) {
  ...
  if (family === "fog") return "Fog";
  if (family === "clear") return "Clear";
  if (family === "partly-cloudy") return "Partly cloudy";
  if (family === "cloudy") return "Cloudy";
...
function forecastHourPresentation(data, index, options = {}) {
  ...
  const baseCode = isCurrent && truth
    ...
    : profile.code;
  const convective = isCurrent && truth
    ? truth.convective || null
    : !isCurrent
      ? convectiveEvidenceForFutureHour(data, index, profile)
      : null;
  ...
  const feels = isCurrent
    ? Number(current?.apparent_temperature ?? h.apparent_temperature?.[index] ?? temperature)
    : Number(h.apparent_temperature?.[index] ?? temperature);
  const wind = isCurrent
    ? Number(current?.wind_speed_10m ?? h.wind_speed_10m?.[index] ?? 0)
    : Number(h.wind_speed_10m?.[index] ?? 0);
  const gust = isCurrent
    ? Number(current?.wind_gusts_10m ?? h.wind_gusts_10m?.[index] ?? wind)
    : Number(h.wind_gusts_10m?.[index] ?? wind);
  const isDay = isCurrent && truth
    ? Boolean(truth.isDay)
  ...
  return {
    ...
    stormPotential: isCurrent && truth
      ...
      : Boolean(modelStormPotential || convective),
    convective,
    temperature,
    feels,
    wind,
    gust,
    uv: Math.max(0, Number(h.uv_index?.[index]) || 0),
    isDay,
    isCurrent,
    source: convective?.source || (isCurrent && truth ? truth.source : "hourly-forecast"),
    basis: convective?.basis || (isCurrent && truth?.source === "radar-current" ? "observed" : "forecast")
  };
}

function dailyRelevantHourlyIndices(data, dayIndex, options = {}) {
  ...
  const remaining = times
    ...
    .filter((row) => row.time.startsWith(day) && row.ms !== null && (dayIndex !== todayIndex || row.ms >= now - 30 * 60 * 1000));
  if (!options.primary || !remaining.length) return remaining.map((row) => row.index);

  const daytime = remaining.filter((row) => data.hourly?.is_day ? Boolean(data.hourly.is_day[row.index]) : false);
  if (daytime.length >= Math.min(4, remaining.length)) return daytime.map((row) => row.index);
  const waking = remaining.filter((row) => {
    const hour = forecastLocalHour(row.time);
...
function collapseForecastSegments(data, indices, options = {}) {
  ...
  indices.forEach((index) => {
    ...
    if (previous && previous.family === hour.family) {
      ...
      previous.endMs = hour.ms !== null ? hour.ms + 60 * 60 * 1000 : previous.endMs;
      previous.maxPop = Math.max(previous.maxPop, hour.pop);
      previous.maxGust = Math.max(previous.maxGust, hour.gust);
      if (hour.stormPotential) previous.stormPotential = true;
      return;
    }
    segments.push({
      family: hour.family,
      code: hour.code,
      label: hour.storyLabel,
      startIndex: index,
      endIndex: index,
      startMs: hour.ms,
      endMs: hour.ms !== null ? hour.ms + 60 * 60 * 1000 : null,
      maxPop: hour.pop,
      maxGust: hour.gust,
      stormPotential: hour.stormPotential
    });
  });
  return segments;
}

function dailyPrecipitationWindow(data, dayIndex) {
  const indices = dailyRelevantHourlyIndices(data, dayIndex);
  const wet = indices.filter((index) => {
    const hour = forecastHourPresentation(data, index, { isCurrent: index === currentHourlyIndex(data) });
    return hour.precipPrimary || hour.pop >= DAILY_PRECIP_THREE_HOURS_POP;
  });
  ...
  const group = groups.sort((a, b) => {
    const aHours = a.endIndex - a.startIndex + 1;
    const bHours = b.endIndex - b.startIndex + 1;
    const aPop = Math.max(...Array.from({ length: aHours }, (_, offset) => Number(data.hourly.precipitation_probability?.[a.startIndex + offset] || 0)));
    const bPop = Math.max(...Array.from({ length: bHours }, (_, offset) => Number(data.hourly.precipitation_probability?.[b.startIndex + offset] || 0)));
    return (bHours * 100 + bPop) - (aHours * 100 + aPop);
  })[0];
  const hours = Array.from({ length: group.endIndex - group.startIndex + 1 }, (_, offset) =>
    forecastHourPresentation(data, group.startIndex + offset, { isCurrent: group.startIndex + offset === currentHourlyIndex(data) })
  );
  const strongest = hours.slice().sort((a, b) =>
    (precipRank(b.code) * 100 + b.pop) - (precipRank(a.code) * 100 + a.pop)
  )[0];
  const endTime = data.hourly.time?.[group.endIndex + 1];
  return {
    ...group,
    startTime: data.hourly.time?.[group.startIndex] || "",
    endTime: endTime || "",
    hours: hours.length,
    pop: Math.max(...hours.map((hour) => hour.pop)),
    kind: strongest?.family === "storm" || strongest?.stormPotential ? "storm" : strongest?.precipKind || "rain"
  };
}

function dailyTimingPhrase(data, dayIndex) {
  const relevant = dailyRelevantHourlyIndices(data, dayIndex);
  if (!relevant.length) return "";
  const convective = nwsConvectiveWindowForIndices(data, relevant);
  if (convective) return nwsConvectiveTimingPhrase(data, convective, relevant);
  const precip = dailyPrecipitationWindow(data, dayIndex);
  if (precip) {
    const noun = precip.kind === "storm" ? "Storms" : precip.kind === "snow" ? "Snow" : "Rain";
    const firstRelevant = relevant[0];
    const lastRelevant = relevant.at(-1);
    const startsWithPrecip = precip.startIndex <= firstRelevant + 1;
    const runsToEnd = precip.endIndex >= lastRelevant - 1;
    if (startsWithPrecip && !runsToEnd && precip.endTime) return `${noun} ending near ${formatTime(precip.endTime)}`;
    if (!startsWithPrecip) {
      const timing = formatTime(precip.startTime);
      const possibility = precip.pop < RAIN_LIKELY_POP ? " possible" : "";
      return `${noun}${possibility} after ${timing}`;
    }
    return `${noun} much of the day`;
  }

  const primary = dailyRelevantHourlyIndices(data, dayIndex, { primary: true });
  const segments = collapseForecastSegments(data, primary);
  const first = segments[0];
  const transition = segments.slice(1).find((segment) => segment.endIndex - segment.startIndex >= 1) || segments[1];
  if (first && transition?.startMs !== null && transition?.startMs !== undefined) {
    const time = formatForecastMs(transition.startMs, data);
    if (["cloudy", "partly-cloudy"].includes(first.family) && transition.family === "clear") return `Clearing near ${time}`;
    if (first.family === "cloudy" && transition.family === "partly-cloudy") return `Some clearing near ${time}`;
    if (["clear", "partly-cloudy"].includes(first.family) && transition.family === "cloudy") return `Clouds increase near ${time}`;
    if (first.family === "fog" && transition.family !== "fog") return `Fog lifting near ${time}`;
  }

  const windUnit = state.unit === "fahrenheit" ? "mph" : "km/h";
  const windyThreshold = windUnit === "mph" ? 25 : 40;
  const windyIndex = relevant.find((index) => Number(data.hourly.wind_gusts_10m?.[index] || 0) >= windyThreshold);
  if (windyIndex >= 0 && windyIndex > relevant[0]) return `Wind picks up near ${formatTime(data.hourly.time[windyIndex])}`;
  return "";
}
...
function forecastDayPresentation(data, dayIndex, options = {}) {
  ...
  primaryIndices.forEach((index) => {
    ...
    skyCounts[code] = (skyCounts[code] || 0) + 1;
  });
  const modalSky = Object.keys(skyCounts).sort((a, b) => skyCounts[b] - skyCounts[a])[0];
  const fallbackCode = isPrecipCode(data.daily?.weather_code?.[dayIndex])
  ...
  const convective = nwsConvectiveWindowForIndices(data, relevantIndices);
  const stormPotential = !isThunderCode(code) && Boolean(
    hasThunderPotentialForDay(data, dayIndex, code) || convective
  );
  return {
    dayIndex,
    code,
    label: dailyConditionLabel(code),
    storyLabel: forecastStoryCondition(code),
    family: forecastConditionFamily(code),
    timing,
    precip: precipProfile,
    stormPotential,
    convective,
    materialEvent,
    primaryIndices
...
function representativeDailyCode(data, dayIndex) {
  return forecastDayPresentation(data, dayIndex).code;
}

function hasThunderPotentialForDay(data, dayIndex, shownCode) {
  if (isThunderCode(shownCode)) return false;
  const indices = dailyRelevantHourlyIndices(data, dayIndex);
  return indices.some((index) => forecastHourPresentation(data, index, {
...
function forecastDataTimeRange(data = state.forecast) {
  const times = data?.hourly?.time || [];
  const first = times.length ? parseForecastTimestamp(times[0], data) : null;
  const last = times.length ? parseForecastTimestamp(times[times.length - 1], data) : null;
  return { first, last };
}

// Forecast rows are stamped in the place's local time, but the app must keep
// moving with the real clock after a response arrives. Falling back to the
// response clock only keeps historical fixtures and retained snapshots usable.
function forecastEvaluationNowMs(data = state.forecast, wallNow = Date.now()) {
  const liveNow = Number(wallNow);
  const range = forecastDataTimeRange(data);
  const liveFallsInsideForecast = Number.isFinite(liveNow) &&
    Number.isFinite(range.first) && Number.isFinite(range.last) &&
    liveNow >= range.first - FORECAST_CACHE_FALLBACK_MAX_AGE_MS &&
    liveNow <= range.last + 2 * 60 * 60 * 1000;
  if (liveFallsInsideForecast) return liveNow;
  return parseForecastTimestamp(data?.current?.time, data) ?? (Number.isFinite(liveNow) ? liveNow : Date.now());
}

function forecastNowMs(data = state.forecast) {
  return forecastEvaluationNowMs(data);
}

function forecastLocalDateAtMs(data = state.forecast, ms = forecastNowMs(data)) {
...
function forecastLocalDate(data = state.forecast, offsetDays = 0) {
  const base = forecastLocalDateAtMs(data) || datePart(data?.current?.time) || data?.daily?.time?.[0] || datePart(Date.now());
...
function forecastCurrentHour(data = state.forecast) {
  const shifted = new Date(forecastNowMs(data) + forecastOffsetMs(data));
  return Number.isNaN(shifted.getTime())
    ? Math.floor(forecastLocalHour(data?.current?.time))
    : shifted.getUTCHours();
}
...
async function loadPlace(place, force = false) {
  ...
  try {
    ...
    if (canKeepPreviousPlace) {
      ...
      renderSavedPlaces();
      updateMapPlace();
      renderForecast(previousForecast, previousPlace, {
        refreshMap: false,
  ...
  // Unsupported and failed are kept distinct from a confirmed empty alert list.
  if (!placeSupportsNwsAlerts(nextPlace)) {
    if (requestId !== loadPlaceRequestSeq || !samePlanPlace(nextPlace, state.activePlace)) return;
    setAlertsLoading(nextPlace);
  } else {
    try {
...
let forecastClockRenderKey = "";
let autoForecastRefreshInFlight = false;
const FOREGROUND_STALE_MS = 8 * 60 * 1000;
const VIEW_RESET_IDLE_MS = 20 * 60 * 1000;

function liveForecastRenderKey(data = state.forecast) {
  if (!data) return "";
  const now = forecastNowMs(data);
  return [
    currentHourlyIndex(data),
    forecastDailyIndex(data),
    Math.floor(now / FORECAST_CLOCK_BUCKET_MS)
  ].join(":");
}
...
function refreshForecastForLiveClock({ forceRender = false, skipNetwork = false } = {}) {
  ...
  if (forceRender || nextKey !== forecastClockRenderKey) {
    ...
    state.weatherTruth = null;
    renderForecast(state.forecast, state.activePlace, {
      only: ["hero", "launch", "glance", "nowcast", "hourly", "daily", "sky", "continuity"],
      refreshMap: false,
      resetBriefing: false,
      reason: "live-clock"
    });
...
function scheduleForecastClock() {
  if (forecastClockTimer) clearTimeout(forecastClockTimer);
  const delay = 60 * 1000 - (Date.now() % (60 * 1000)) + 80;
  forecastClockTimer = setTimeout(() => {
    refreshForecastForLiveClock();
    scheduleForecastClock();
  }, delay);
}

function initForecastClock() {
  forecastClockRenderKey = liveForecastRenderKey(state.forecast);
  scheduleForecastClock();
}
...
function resetTransientViewToForecastTop() {
  ...
  if (els.forecastReceiptSheet && !els.forecastReceiptSheet.hidden) closeForecastReceipt();
...
const FORECAST_CONFIDENCE_CACHE_FALLBACK_MS = 3 * 60 * 60 * 1000;
const FORECAST_CONFIDENCE_FORCE_FLOOR_MS = 10 * 60 * 1000;
const FORECAST_CONFIDENCE_FETCH_TIMEOUT_MS = 6500;
const FORECAST_CONFIDENCE_MODELS = Object.freeze([
...
function forecastConfidenceCacheKey(place, unit = state.unit) {
...
function normalizeForecastConfidenceGuidance(json, place) {
  const times = Array.isArray(json?.hourly?.time) ? json.hourly.time : [];
  const placeKey = continuityPlaceKey(place);
  const fetchedAtMs = Date.now();
  const sources = FORECAST_CONFIDENCE_MODELS.map((model) => {
    const temperature = json?.hourly?.[`temperature_2m_${model.suffix}`];
    const precipitationProbability = json?.hourly?.[`precipitation_probability_${model.suffix}`];
    const precipitation = json?.hourly?.[`precipitation_${model.suffix}`];
    const weatherCode = json?.hourly?.[`weather_code_${model.suffix}`];
    const gust = json?.hourly?.[`wind_gusts_10m_${model.suffix}`];
    const usable = [temperature, precipitationProbability, precipitation, weatherCode, gust]
      .some((values) => Array.isArray(values) && values.some((value) => forecastConfidenceNumber(value) !== null));
    const hours = usable ? times.map((time, index) => ({
      atMs: parseForecastTimestamp(time, json),
      temperature: forecastConfidenceNumber(temperature?.[index]),
      precipProbability: forecastConfidenceNumber(precipitationProbability?.[index]),
      precipitation: forecastConfidenceNumber(precipitation?.[index]),
      weatherCode: forecastConfidenceNumber(weatherCode?.[index]),
      gust: forecastConfidenceNumber(gust?.[index])
    })).filter((hour) => hour.atMs !== null && Number.isFinite(hour.atMs)) : [];
    return {
      id: model.id,
      model: model.name,
      status: hours.length >= 12 ? "ready" : "missing",
      hours
    };
  });
  ...
  return {
    ...
    placeKey,
    fetchedAtMs,
    expiresAtMs: fetchedAtMs + FORECAST_CONFIDENCE_CACHE_MAX_AGE_MS,
    status: readyCount === sources.length ? "ready" : readyCount ? "partial" : "failed",
    horizonHours: 72,
    units: {
      temperature: json?.hourly_units?.[`temperature_2m_${FORECAST_CONFIDENCE_MODELS[0].suffix}`] || "",
      wind: json?.hourly_units?.[`wind_gusts_10m_${FORECAST_CONFIDENCE_MODELS[0].suffix}`] || "",
      precipitation: json?.hourly_units?.[`precipitation_${FORECAST_CONFIDENCE_MODELS[0].suffix}`] || ""
    },
    sources,
    limitations: readyCount < sources.length ? [`${readyCount} of ${sources.length} model families returned usable guidance.`] : []
  };
}

async function fetchForecastConfidenceGuidance(place) {
  const params = new URLSearchParams({
  ...
  const json = await fetchJsonWithTimeout(`https://api.open-meteo.com/v1/forecast?${params}`, FORECAST_CONFIDENCE_FETCH_TIMEOUT_MS);
  const bundle = normalizeForecastConfidenceGuidance(json, place);
  try {
    localStorage.setItem(forecastConfidenceCacheKey(place), JSON.stringify({ savedAt: bundle.fetchedAtMs, bundle }));
  } catch {
    // Comparison guidance remains usable even when browser storage is full or unavailable.
  }
  return bundle;
}

function sharedForecastConfidenceGuidanceRequest(place) {
  // Cache storage can be coordinate-scoped, but in-flight work must also be
  // scoped to the exact place identity because normalized bundles retain that
  // identity for strict cross-place isolation.
  const key = `${forecastConfidenceCacheKey(place)}:${continuityPlaceKey(place)}`;
  const existing = forecastConfidenceInflightByKey.get(key);
  if (existing) return existing;
  const request = fetchForecastConfidenceGuidance(place).finally(() => {
    if (forecastConfidenceInflightByKey.get(key) === request) forecastConfidenceInflightByKey.delete(key);
  });
  forecastConfidenceInflightByKey.set(key, request);
  return request;
}

function bindForecastConfidenceGuidance(data, place, bundle, status = bundle?.status) {
  if (!data || !place || !bundle || bundle.placeKey !== continuityPlaceKey(place)) return false;
  forecastConfidenceGuidanceByForecast.set(data, { ...bundle, status: status || bundle.status });
  return true;
}

function renderForecastConfidenceSurfaces(data, place) {
  if (!data || state.forecast !== data || !samePlanPlace(state.activePlace, place)) return;
  ...
  if (typeof refreshOpenDayDetailMemorySurfaces === "function") refreshOpenDayDetailMemorySurfaces();
  syncNativeWidgetSnapshot(data, place, truth);
}

async function startForecastConfidenceProbe(place, data, force = false) {
  if (!place || !data || typeof forecastConfidencePresentation !== "function") return;
  const sequence = ++forecastConfidenceProbeSequence;
  const cached = readForecastConfidenceCache(place);
  const cachedAge = cached ? Date.now() - cached.savedAt : Infinity;
  let cachedBound = false;
  if (cached) {
    cachedBound = bindForecastConfidenceGuidance(
      data,
      place,
      cached.bundle,
      cachedAge <= FORECAST_CONFIDENCE_CACHE_MAX_AGE_MS ? cached.bundle.status : "stale"
    );
    if (cachedBound && state.forecast === data && samePlanPlace(state.activePlace, place)) {
      renderForecastConfidenceSurfaces(data, place);
    }
  }
  const cacheIsFresh = cachedBound && cachedAge <= FORECAST_CONFIDENCE_CACHE_MAX_AGE_MS;
  const forceIsFloored = force && cachedBound && cachedAge <= FORECAST_CONFIDENCE_FORCE_FLOOR_MS;
  if ((!force && cacheIsFresh) || forceIsFloored) return;
  try {
    const bundle = await sharedForecastConfidenceGuidanceRequest(place);
    if (
      sequence !== forecastConfidenceProbeSequence ||
      state.forecast !== data ||
      !samePlanPlace(state.activePlace, place)
    ) return;
    bindForecastConfidenceGuidance(data, place, bundle);
    renderForecastConfidenceSurfaces(data, place);
  } catch {
...
function forecastAgeLabel(ageMs) {
  const minutes = Math.max(0, Math.round(Number(ageMs) / 60000));
...
function forecastPulseDaySnapshot(data, dayIndex) {
  ...
  const materialEvent = day.materialEvent || null;
  const indices = dailyRelevantHourlyIndices(data, dayIndex);
  ...
  return {
    ...
    timing: day.timing || "",
    eventId: materialEvent?.id || null,
    eventKind: materialEvent?.kind || null,
    eventHeadline: materialEvent?.headline || null,
    eventSupport: materialEvent?.support || null,
    eventStartMs: Number.isFinite(Number(materialEvent?.startMs)) ? Number(materialEvent.startMs) : null,
    eventEndMs: Number.isFinite(Number(materialEvent?.endMs)) ? Number(materialEvent.endMs) : null,
    precipStartMs: Number.isFinite(Number(materialEvent?.startMs))
      ? Number(materialEvent.startMs)
      : precipWindow?.startTime ? parseForecastTimestamp(precipWindow.startTime, data) : null,
    precipEndMs: Number.isFinite(Number(materialEvent?.endMs))
      ? Number(materialEvent.endMs)
      : precipWindow?.endTime ? parseForecastTimestamp(precipWindow.endTime, data) : null,
    hourly
  };
}
...
function forecastPulseChange(current, previous, tempUnit, windUnit) {
  ...
  // Treat a legacy precipitation snapshot as the same material event during
  // the one-time schema migration. Otherwise the first v8 comparison can
  // incorrectly announce that an already-known rain window is "now" present.
  const currentEvent = current.eventId || Number.isFinite(Number(current.precipStartMs)) ? current : null;
  const previousEvent = previous.eventId || Number.isFinite(Number(previous.precipStartMs)) ? previous : null;
  if (currentEvent && !previousEvent) {
    add(120, "event", `${current.eventHeadline || "Weather change"} is now in the forecast`, current.eventSupport || current.timing || "The next meaningful weather window changed.");
  } else if (!currentEvent && previousEvent) {
    add(118, "event", `${previous.eventKind === "storm" ? "Storms" : previous.eventKind === "snow" ? "Snow" : "Rain"} backed off`, "The previously highlighted weather window is no longer supported by the latest forecast.");
  } else if (currentEvent && previousEvent && current.eventId && previous.eventId && current.eventId !== previous.eventId) {
    add(116, "event", "The main weather window changed", `${current.eventHeadline || current.timing || "A new weather window"} replaced ${String(previous.eventHeadline || previous.timing || "the earlier signal").toLowerCase()}.`);
  }
  const timingDelta = continuityDelta(current.eventStartMs ?? current.precipStartMs, previous.eventStartMs ?? previous.precipStartMs);
  if (timingDelta !== null && Math.abs(timingDelta) >= 90 * 60 * 1000 && Math.min(current.rainMax || 0, previous.rainMax || 0) >= 25) {
    ...
    const eventName = current.eventKind === "storm" ? "Storms" : current.eventKind === "snow" ? "Snow" : "Rain";
    add(100 + Math.round(Math.abs(timingDelta) / 3600000), "timing", `${eventName} moved ${later ? "later" : "earlier"}`, `Now near ${formatTime(current.eventStartMs ?? current.precipStartMs)}, previously near ${formatTime(previous.eventStartMs ?? previous.precipStartMs)}.`);
  }
...
function forecastConfidenceClaim(data, truth = weatherTruth(data)) {
  const nowMs = forecastNowMs(data);
  const event = forecastMaterialEvent(data, { truth });
  if (event && Number(event.endMs) > nowMs && Number(event.startMs) <= nowMs + 72 * 60 * 60 * 1000) {
    const eventKind = String(event.kind || "weather");
    if (!["rain", "storm", "snow", "ice", "wind"].includes(eventKind)) return null;
    const kind = ["rain", "storm", "snow", "ice"].includes(eventKind)
      ? "precip-window"
      : eventKind === "wind" ? "wind" : "condition";
    return {
      id: event.id || `${kind}:${event.startMs}:${event.endMs}`,
      kind,
      headline: event.headline || event.label || "The next meaningful weather window",
      startMs: Number(event.startMs),
      endMs: Number(event.endMs),
      canonical: {
        eventKind,
        windUnit: state.unit === "fahrenheit" ? "mph" : "km/h",
        onsetMs: Number(event.startMs),
        tempMin: null,
        tempMax: null,
        gustMax: null
      }
  ...
  const endMs = Math.min(
    nowMs + 12 * 60 * 60 * 1000,
    parseForecastTimestamp(data?.hourly?.time?.at(-1), data) || nowMs + 12 * 60 * 60 * 1000
  );
  const timing = nearcastRelativeTiming(data, nowMs, endMs);
  return {
    id: `dry-window:${nowMs}:${endMs}`,
    kind: "dry-window",
    headline: `Dry through ${timing.endTimeLabel} ${timing.endDayLabel}`,
    startMs: nowMs,
    endMs,
    canonical: { eventKind: "dry", onsetMs: null, tempMin: null, tempMax: null, gustMax: null }
  };
}

function forecastConfidenceWetHour(hour, precipUnit) {
  const amountThreshold = precipUnit === "inch" ? 0.01 : 0.25;
  const code = Number(hour?.weatherCode);
  const precipCode = (code >= 51 && code <= 86) || code >= 95;
  return Number(hour?.precipitation || 0) >= amountThreshold || (
    Number(hour?.precipProbability || 0) >= 35 && precipCode
  );
}

function forecastConfidencePrecipKind(hours) {
  const codes = (hours || []).map((hour) => Number(hour?.weatherCode)).filter(Number.isFinite);
  if (codes.some((code) => code >= 95 && code <= 99)) return "storm";
  if (codes.some((code) => [56, 57, 66, 67].includes(code))) return "ice";
  if (codes.some((code) => (code >= 71 && code <= 77) || code === 85 || code === 86)) return "snow";
  return "rain";
}

function forecastConfidenceSourceSignal(source, bundle, claim) {
  const hours = Array.isArray(source?.hours) ? source.hours : [];
  const margin = claim.kind === "precip-window" ? 8 * 60 * 60 * 1000 : 0;
  const scoped = hours.filter((hour) => (
    Number(hour.atMs) >= Number(claim.startMs) - margin &&
    Number(hour.atMs) < Number(claim.endMs) + margin
  ));
  const wet = scoped.filter((hour) => forecastConfidenceWetHour(hour, bundle?.units?.precipitation));
  const groups = [];
  wet.forEach((hour) => {
    const previous = groups.at(-1);
    if (previous && Number(hour.atMs) - Number(previous.at(-1).atMs) <= 90 * 60 * 1000) previous.push(hour);
    else groups.push([hour]);
  });
  const group = groups.sort((a, b) => (
    Math.abs(Number(a[0]?.atMs) - Number(claim.startMs)) -
    Math.abs(Number(b[0]?.atMs) - Number(claim.startMs))
  ))[0] || null;
  const hourMs = 60 * 60 * 1000;
  const exactWindow = hours.filter((hour) => (
    Number(hour.atMs) + hourMs > Number(claim.startMs) &&
    Number(hour.atMs) < Number(claim.endMs)
  ));
  const temperatures = exactWindow.map((hour) => forecastConfidenceNumber(hour.temperature)).filter((value) => value !== null);
  const gusts = exactWindow.map((hour) => forecastConfidenceNumber(hour.gust)).filter((value) => value !== null);
  const coverageStartMs = exactWindow.length ? Math.min(...exactWindow.map((hour) => Number(hour.atMs))) : null;
  const coverageEndMs = exactWindow.length ? Math.max(...exactWindow.map((hour) => Number(hour.atMs))) + hourMs : null;
  const expectedBucketCount = Math.max(
    1,
    Math.ceil(Number(claim.endMs) / hourMs) - Math.floor(Number(claim.startMs) / hourMs)
  );
  const coverageToleranceMs = 5 * 60 * 1000;
  const coverageComplete = Number.isFinite(coverageStartMs) && Number.isFinite(coverageEndMs) &&
    exactWindow.length >= expectedBucketCount &&
    coverageStartMs <= Number(claim.startMs) + coverageToleranceMs &&
    coverageEndMs >= Number(claim.endMs) - coverageToleranceMs;
  const precipitationEvidenceCount = exactWindow.filter((hour) => (
    forecastConfidenceNumber(hour.precipProbability) !== null ||
    forecastConfidenceNumber(hour.precipitation) !== null ||
    forecastConfidenceNumber(hour.weatherCode) !== null
  )).length;
  const relevantEvidenceAvailable = claim.kind === "temperature"
    ? temperatures.length === exactWindow.length
    : claim.kind === "wind"
      ? gusts.length === exactWindow.length
      : precipitationEvidenceCount === exactWindow.length;
  return {
    id: source.id,
    status: source.status === "ready" && coverageComplete && relevantEvidenceAvailable ? "ready" : "missing",
    placeKey: bundle.placeKey,
    issuedAtMs: bundle.fetchedAtMs,
    precipitation: group ? {
      kind: forecastConfidencePrecipKind(group),
      startMs: Number(group[0].atMs),
      endMs: Number(group.at(-1).atMs) + 60 * 60 * 1000
    } : null,
    temperature: temperatures.length ? { min: Math.min(...temperatures), max: Math.max(...temperatures) } : null,
    wind: gusts.length ? { gustMax: Math.max(...gusts) } : null
  };
}

function forecastConfidenceHistory(data, claim, place = state.activePlace) {
  const currentSavedAt = Number(forecastProvenance(data).savedAt);
  const claimStartMs = forecastConfidenceNumber(claim?.startMs);
  const claimDayIndex = claimStartMs === null ? -1 : (data?.daily?.time || []).findIndex((date, index, dates) => {
    const startMs = parseForecastTimestamp(date, data);
    const nextMs = dates[index + 1] ? parseForecastTimestamp(dates[index + 1], data) : startMs + 24 * 60 * 60 * 1000;
    return Number.isFinite(startMs) && Number.isFinite(nextMs) && claimStartMs >= startMs && claimStartMs < nextMs;
  });
  const claimDate = claimDayIndex >= 0 ? data.daily.time[claimDayIndex] : null;
  const claimKind = String(claim?.canonical?.eventKind || "");
  const compatibleKind = (day) => {
    const dayKind = String(day?.eventKind || "");
    if (!claimKind || !dayKind) return false;
    return dayKind === claimKind;
  };
  return forecastPulseRuns(data, place).filter((run) => (
    !Number.isFinite(currentSavedAt) || Number(run.checkedAt) < currentSavedAt
  )).map((run) => {
    const day = (run.days || []).find((candidate) => (
      candidate?.date === claimDate &&
      compatibleKind(candidate) &&
      forecastConfidenceNumber(candidate.precipStartMs) !== null
    ));
    if (!day) return null;
    return {
      placeKey: run.placeKey,
      checkedAtMs: Number(run.checkedAt),
      precipitationStartMs: forecastConfidenceNumber(day?.precipStartMs)
    };
  }).filter(Boolean);
}

function forecastConfidenceObservation(data, place = state.activePlace) {
  const radar = radarSignalForForecastData(data);
  if (!radar) return null;
  return {
    placeKey: continuityPlaceKey(place),
    status: ["active", "clear", "nearby"].includes(String(radar.phase || "")) ? "ready" : "unavailable",
    source: String(radar.source || "weather radar"),
    observedAtMs: Number.isFinite(Number(radar.timestamp)) ? Number(radar.timestamp) : null,
    phase: radar.phase === "active" ? "active-rain" : String(radar.phase || "unavailable")
  };
}

function forecastConfidenceSourceList(names) {
...
function nearcastForecastConfidenceForWindow(options = {}) {
  const data = options.data || state.forecast;
  const place = options.place || state.activePlace;
  const truth = options.truth || state.weatherTruth || weatherTruth(data);
  if (!data || !place || typeof forecastConfidencePresentation !== "function") return null;
  const bundle = forecastConfidenceGuidanceByForecast.get(data);
  if (!bundle || bundle.placeKey !== continuityPlaceKey(place)) return null;
  const claim = options.claim || forecastConfidenceClaim(data, truth);
  if (!claim) return null;
  const requestedStartMs = forecastConfidenceNumber(options.startMs);
  const requestedEndMs = forecastConfidenceNumber(options.endMs);
  const providerSignals = bundle.sources.map((source) => forecastConfidenceSourceSignal(source, bundle, claim));
  const confidence = forecastConfidencePresentation({
    data,
    place: { key: continuityPlaceKey(place), name: placeLabel(place) },
    nowMs: forecastNowMs(data),
    claim,
    window: {
      startMs: requestedStartMs ?? claim.startMs,
      endMs: requestedEndMs ?? claim.endMs
    },
    providerSignals,
    expectedProviders: FORECAST_CONFIDENCE_MODELS.map((model) => model.id),
    history: forecastConfidenceHistory(data, claim, place),
    observation: forecastConfidenceObservation(data, place),
    canonical: { cacheFallback: forecastProvenance(data).cacheFallback },
    guidanceStatus: bundle.status
  });
  return confidence ? { ...confidence, guidanceFetchedAtMs: bundle.fetchedAtMs } : null;
}

function nearcastForecastConfidence(data = state.forecast, truth = state.weatherTruth || weatherTruth(data), place = state.activePlace) {
...
function nearcastForecastConfidenceForDay(data, dayIndex, day = null, place = state.activePlace) {
  const date = data?.daily?.time?.[dayIndex];
  if (!date) return null;
  const startOfDay = parseForecastTimestamp(date, data);
  const nextDate = data?.daily?.time?.[dayIndex + 1];
  const endOfDay = nextDate ? parseForecastTimestamp(nextDate, data) : startOfDay + 24 * 60 * 60 * 1000;
  const startMs = dayIndex === forecastDailyIndex(data) ? Math.max(forecastNowMs(data), startOfDay) : startOfDay;
  if (!Number.isFinite(startMs) || !Number.isFinite(endOfDay) || startMs >= endOfDay) return null;
  const presentation = day || forecastDayPresentation(data, dayIndex);
  const event = presentation?.materialEvent || null;
  let claim;
  if (event && ["rain", "storm", "snow", "ice", "wind"].includes(String(event.kind || ""))) {
    claim = {
      id: event.id || `day-event:${date}`,
      kind: event.kind === "wind" ? "wind" : "precip-window",
      headline: event.headline || event.label || "The day's main weather window",
      startMs: Math.max(startMs, Number(event.startMs)),
      endMs: Math.min(endOfDay, Number(event.endMs)),
      canonical: {
    ...
    claim = {
      id: `temperature:${date}`,
      kind: "temperature",
      headline: `High near ${Math.round(Number(data?.daily?.temperature_2m_max?.[dayIndex]) || 0)}${degree(state.unit === "fahrenheit" ? "F" : "C")}`,
      startMs,
      endMs: endOfDay,
      canonical: { eventKind: "temperature" }
    };
  }
...
function forecastTrustPresentation(data = state.forecast, truth = weatherTruth(data), options = {}) {
  ...
  const age = ageMs === null ? "time unavailable" : forecastAgeLabel(ageMs);
  const stale = provenance.cacheFallback && ageMs !== null && ageMs >= 90 * 60 * 1000;
  const radar = radarSignalForForecastData(data);
  ...
  if (alerts.state === "ready") {
    ...
    alertMeta = "The weather forecast still works, but Nearcast could not verify active NWS alerts.";
  } else if (alerts.state === "unsupported") {
    alertLabel = "Official alerts not available here";
  ...
  if (confidenceReady && !provenance.cacheFallback && alerts.state !== "failed") {
    ...
    const timingDetail = Number.isFinite(timingRange) && timingRange > 0
      ...
      : "";
    const sourceNames = (agreement.providerIds || []).map((id) => (
      FORECAST_CONFIDENCE_MODELS.find((model) => model.id === id)?.name || id
    ));
  ...
  return {
    ...
    triggerMeta,
    age,
    freshness: savedAt ? `Checked ${age}` : "Update time unavailable",
    freshnessMeta: provenance.cacheFallback
      ? "Nearcast kept the last usable forecast because the live refresh did not complete."
      : "Nearcast refreshes while the app is active and after returning to it.",
    source,
    sourceMeta,
    evidence,
    evidenceMeta,
...
function renderForecastTrust(data = state.forecast, truth = state.weatherTruth || weatherTruth(data)) {
  if (!els.forecastReceiptTrigger) return;
  const activePlaceId = state.activePlace?.id || null;
  if (!data || !activePlaceId || state.forecastPlaceId !== activePlaceId) {
    els.forecastReceiptTrigger.hidden = true;
    if (els.forecastReceiptSheet && !els.forecastReceiptSheet.hidden) closeForecastReceipt();
    return;
  }
  const receipt = forecastTrustPresentation(data, truth);
  els.forecastReceiptTrigger.hidden = false;
  ...
  els.forecastReceiptOverviewMeta.textContent = receipt.overview;
  els.forecastReceiptFreshness.textContent = receipt.freshness;
  els.forecastReceiptFreshnessMeta.textContent = receipt.freshnessMeta;
  els.forecastReceiptSource.textContent = receipt.source;
  ...
  els.forecastReceiptFootnote.textContent = receipt.footnote;
}

function openForecastReceipt() {
  if (
    !state.forecast ||
    !state.activePlace ||
    state.forecastPlaceId !== state.activePlace.id ||
    !els.forecastReceiptSheet ||
    !els.forecastReceiptBackdrop
  ) return;
  renderForecastTrust();
  els.forecastReceiptBackdrop.hidden = false;
  els.forecastReceiptSheet.hidden = false;
  els.forecastReceiptTrigger?.setAttribute("aria-expanded", "true");
  showSheet(els.forecastReceiptBackdrop, els.forecastReceiptSheet, {
    onPullDismiss: closeForecastReceipt,
    resetScroll: true
  });
  document.body.style.overflow = "hidden";
}

function closeForecastReceipt() {
  if (!els.forecastReceiptSheet || !els.forecastReceiptBackdrop || els.forecastReceiptSheet.hidden) return;
  els.forecastReceiptBackdrop.classList.remove("show");
  els.forecastReceiptSheet.classList.remove("show");
  els.forecastReceiptTrigger?.setAttribute("aria-expanded", "false");
  const keepLocked = Boolean(document.querySelector(
    ".day-sheet.show:not(#forecastReceiptSheet), .ai-sheet.show, .memory-sheet.show"
  )) || mapState.immersive;
  document.body.style.overflow = keepLocked ? "hidden" : "";
  setTimeout(() => {
    els.forecastReceiptBackdrop.hidden = true;
    els.forecastReceiptSheet.hidden = true;
  }, 260);
}
...
function buildForecastRenderContext(data, place) {
  ...
  const tempUnit = state.unit === "fahrenheit" ? "F" : "C";
  const windUnit = state.unit === "fahrenheit" ? "mph" : "km/h";
  const presentation = buildForecastPresentation(data, { truth, tempUnit, windUnit });
  const todayIndex = presentation.todayIndex;
...
function applyForecastRenderState(ctx, lanes) {
  ...
  state.forecastPlaceId = ctx.place?.id || null;
  state.forecastUnit = state.unit;
  state.weatherTruth = ctx.truth;
  state.forecastPresentation = ctx.presentation;
...
function renderForecastCurrentReadouts(ctx) {
  ...
  const savedAt = forecastProvenance(ctx.data).savedAt;
  els.updatedAt.textContent = savedAt
    ? `Updated ${forecastAgeLabel(Date.now() - savedAt)}`
    : `Forecast time ${formatTime(ctx.current.time)}`;
}
...
function renderForecastPlan(ctx) {
  ...
  renderGoodOutdoorWindow(ctx.data);
...
function renderForecastLists(ctx, lanes) {
  if (lanes.hourly) renderHourly(ctx.data, ctx.tempUnit, ctx.truth, ctx.presentation);
  if (lanes.daily) renderDaily(ctx.data, ctx.tempUnit, ctx.precipUnit, ctx.presentation);
}
...
function renderForecast(data, place, options = {}) {
  ...
  forecastClockRenderKey = liveForecastRenderKey(data);
...
function forecastStoryPrecipWindow(data, indices, truth = null) {
  ...
  const lastIndex = best.hours.at(-1).index;
  return {
    startIndex: best.hours[0].index,
    endIndex: lastIndex,
    startMs: best.hours[0].ms,
    endMs: data.hourly.time?.[lastIndex + 1]
      ? parseForecastTimestamp(data.hourly.time[lastIndex + 1], data)
      : best.hours.at(-1).ms + 60 * 60 * 1000,
    hours: best.hours.length,
    pop: Math.max(...best.hours.map((hour) => hour.pop)),
    kind: strongest.family === "storm" || strongest.stormPotential ? "storm" : strongest.precipKind
  };
}
...
function forecastTransitionSentence(data, segments, periodLabel) {
  ...
  if (first.family === "cloudy" && transition.family === "partly-cloudy") {
    return `Cloudy through ${at}, then some clearing.`;
  }
  if (["clear", "partly-cloudy"].includes(first.family) && transition.family === "cloudy") {
    return `${first.label} through ${at}, then clouds increase.`;
  }
  if (first.family === "fog" && transition.family !== "fog") return `Fog through ${at}, then improving.`;
...
function buildForecastStory(data, tempUnit, windUnit, truth = weatherTruth(data)) {
  ...
  const todayHigh = Math.round(data.daily.temperature_2m_max?.[todayIndex] ?? current.temperature_2m);
  const overnightLow = Math.round(data.daily.temperature_2m_min?.[tomorrowIndex] ?? data.daily.temperature_2m_min?.[todayIndex] ?? current.temperature_2m);
  const tomorrowHigh = Math.round(data.daily.temperature_2m_max?.[tomorrowIndex] ?? todayHigh);
  let kicker = "Coming up";
  let horizon = midnightMs;
  let periodLabel = "for the rest of the day";
  let temperatureSentence = "";

  if (hour < 12) {
    kicker = "This morning's outlook";
    horizon = sixPmMs;
    periodLabel = "through the afternoon";
    temperatureSentence = `High near ${todayHigh}${degree(tempUnit)} this afternoon.`;
  } else if (hour < 18) {
    kicker = "This afternoon's outlook";
    horizon = midnightMs;
    periodLabel = "for the rest of the day";
    temperatureSentence = `Cooling toward ${overnightLow}${degree(tempUnit)} overnight.`;
  } else {
    kicker = hour >= 21 || !truth.isDay ? "Tonight's outlook" : "This evening's outlook";
    horizon = tomorrowNoonMs;
    periodLabel = "through tonight";
    temperatureSentence = `Low near ${overnightLow}${degree(tempUnit)} tonight; tomorrow reaches ${tomorrowHigh}${degree(tempUnit)}.`;
  }
...
// One meaningful weather event is shared by every forecast surface. The
// provider rows remain available as evidence, but Home, Hourly, Daily, day
// detail, AI, widgets, and Watch must not independently choose a different
// start time or description for the same rain or storm.
function forecastMaterialEventStory(data, options = {}, truth = weatherTruth(data)) {
  if (options.story) return options.story;
  const dayIndex = Number(options.dayIndex);
  if (!Number.isInteger(dayIndex) || dayIndex < 0) {
    const tempUnit = options.tempUnit || (state.unit === "fahrenheit" ? "F" : "C");
    const windUnit = options.windUnit || (state.unit === "fahrenheit" ? "mph" : "km/h");
    return buildForecastStory(data, tempUnit, windUnit, truth);
  }

  const indices = dailyRelevantHourlyIndices(data, dayIndex);
  if (!indices.length) return null;
  const segments = collapseForecastSegments(data, indices, { truth });
  const precipWindow = forecastStoryPrecipWindow(data, indices, truth);
  const convectiveWindow = nwsConvectiveWindowForIndices(data, indices);
  const transition = forecastStoryTransitionSegment(segments);
  const windUnit = options.windUnit || (state.unit === "fahrenheit" ? "mph" : "km/h");
  const gustThreshold = windUnit === "mph" ? 25 : 40;
  const gustIndex = indices.reduce((best, index) => {
    const gust = Number(data?.hourly?.wind_gusts_10m?.[index]);
    const bestGust = Number(data?.hourly?.wind_gusts_10m?.[best]);
    return Number.isFinite(gust) && (!Number.isFinite(bestGust) || gust > bestGust) ? index : best;
  }, -1);
  const gust = gustIndex >= 0 ? Math.round(Number(data?.hourly?.wind_gusts_10m?.[gustIndex]) || 0) : 0;
  return {
    segments,
    precipWindow,
    convectiveWindow,
...
function forecastMaterialEventId(data, event, truth = null) {
  if (!event || !Number.isFinite(Number(event.startMs))) return "";
  const precipitationKinds = new Set(["rain", "storm", "snow", "ice", "precipitation"]);
  const identityKind = precipitationKinds.has(event.kind) ? "precip" : event.kind || "weather";
  let anchorIndex = Number.isInteger(event.startIndex) ? event.startIndex : -1;

  // Preserve an active precipitation event's identity as the clock advances.
  // Scan backward through the same contiguous wet window instead of assigning
  // a new id to each new current hour.
  if (identityKind === "precip" && anchorIndex >= 0) {
    const isWet = (index) => {
      if (index < 0 || index >= (data?.hourly?.time?.length || 0)) return false;
      const currentIndex = currentHourlyIndex(data);
      const hour = forecastHourPresentation(data, index, {
        isCurrent: index === currentIndex,
        truth: index === currentIndex ? truth : null
      });
      return Boolean(hour.precipPrimary || hour.pop >= DAILY_PRECIP_THREE_HOURS_POP || hour.stormPotential);
    };
    while (anchorIndex > 0 && isWet(anchorIndex - 1)) anchorIndex -= 1;
  }

  const anchorMs = anchorIndex >= 0
    ? parseForecastTimestamp(data?.hourly?.time?.[anchorIndex], data)
    : Number(event.startMs);
  const date = forecastLocalDateAtMs(data, Number.isFinite(anchorMs) ? anchorMs : Number(event.startMs)) || "future";
  if (identityKind !== "precip") {
    const shifted = new Date(Number(event.startMs) + forecastOffsetMs(data));
    const daypart = Math.floor(shifted.getUTCHours() / 6);
    return `${identityKind}:${date}:${daypart}`;
  }

  let ordinal = 0;
  let inside = false;
  for (let index = 0; index <= Math.max(0, anchorIndex); index += 1) {
    const ms = parseForecastTimestamp(data?.hourly?.time?.[index], data);
    if (!Number.isFinite(ms) || forecastLocalDateAtMs(data, ms) !== date) continue;
    const currentIndex = currentHourlyIndex(data);
    const hour = forecastHourPresentation(data, index, {
      isCurrent: index === currentIndex,
      truth: index === currentIndex ? truth : null
    });
    const wet = Boolean(hour.precipPrimary || hour.pop >= DAILY_PRECIP_THREE_HOURS_POP || hour.stormPotential);
    if (wet && !inside) ordinal += 1;
    inside = wet;
  }
  return `${identityKind}:${date}:${Math.max(0, ordinal - 1)}`;
}

function forecastMaterialEventLabel(data, kind, likelihood, startMs, endMs, active = false) {
  const timing = nearcastRelativeTiming(data, startMs, endMs);
  const noun = kind === "storm" ? "Storms" : kind === "snow" ? "Snow" : kind === "ice" ? "Ice" : "Rain";
  if (active) {
    const ending = Number.isFinite(Number(endMs)) && Number(endMs) > forecastNowMs(data) + 45 * 60 * 1000
      ? `; easing near ${timing.endTimeLabel} ${timing.endDayLabel}`
      : "";
    return `${noun} now${ending}`;
  }
  const when = Number(endMs) - Number(startMs) > 75 * 60 * 1000
    ? timing.rangeLabel
    : `around ${timing.timeLabel} ${timing.dayLabel}`;
  return `${noun} ${likelihood} ${when}`;
}

function forecastMaterialEventDailyTiming(data, event) {
  if (!event) return "";
  const noun = event.kind === "storm" ? "Storms" : event.kind === "snow" ? "Snow" : event.kind === "ice" ? "Ice" : event.kind === "wind" ? "Wind" : event.kind === "fog" ? "Fog" : "Rain";
  if (event.active) {
    return Number(event.endMs) > forecastNowMs(data) + 45 * 60 * 1000
      ? `${noun} now; easing near ${event.timing.endTimeLabel}`
      : `${noun} now`;
  }
  if (event.kind === "wind" || event.kind === "fog") return event.headline || event.label || "";
  const duration = Number(event.endMs) - Number(event.startMs);
  const when = duration > 75 * 60 * 1000
    ? `${event.timing.timeLabel}–${event.timing.endTimeLabel}`
    : `near ${event.timing.timeLabel}`;
  return `${noun} ${event.likelihood || "possible"} ${when}`;
}

function forecastMaterialEvent(data, options = {}) {
  if (!data) return null;
  const truth = options.truth || (data === state.forecast ? (state.weatherTruth || weatherTruth(data)) : buildWeatherTruth(data));
  const story = forecastMaterialEventStory(data, options, truth);
  if (!story) return null;
  const base = nearcastPromotedEvent(data, story, truth, options);
  if (!nearcastBriefMaterialEvent(base)) return null;

  const precipWindow = story.precipWindow || null;
  const convectiveWindow = story.convectiveWindow || null;
  const currentIndex = currentHourlyIndex(data);
  const now = forecastNowMs(data);
  let startMs = Number(base.startMs);
  let endMs = Number(base.endMs);
  let startIndex = Number.isInteger(precipWindow?.startIndex) ? precipWindow.startIndex : base.hourlyIndex;
  let endIndex = Number.isInteger(precipWindow?.endIndex) ? precipWindow.endIndex : base.hourlyIndex;
  let kind = base.kind;
  let chance = Number.isFinite(Number(base.chance)) ? Number(base.chance) : null;
  let likelihood = /\blikely\b/i.test(String(base.label || "")) || (chance !== null && chance >= RAIN_LIKELY_POP)
    ? "likely"
    : "possible";
  let headline = base.label;
  let support = "";
  let peakStartMs = startMs;
  let peakEndMs = endMs;
  let peakStartIndex = startIndex;
  let peakEndIndex = endIndex;
  const phases = [];

  if (precipWindow) {
    const firstHour = forecastHourPresentation(data, precipWindow.startIndex, {
      isCurrent: precipWindow.startIndex === currentIndex,
      truth: precipWindow.startIndex === currentIndex ? truth : null
    });
    const onsetKind = firstHour.family === "snow" ? "snow"
      : firstHour.family === "storm" ? "storm"
        : firstHour.precipKind === "snow" ? "snow" : "rain";
    const active = truth?.precip?.phase === "active" && startMs <= now + 30 * 60 * 1000 && endMs > now;
    kind = onsetKind;
    headline = forecastMaterialEventLabel(data, kind, likelihood, startMs, endMs, active);
    if (active && base.basis !== "observed") {
      headline = headline.replace(/^(Rain|Storms|Snow|Ice) now\b/i, "$1 indicated now");
    }
    phases.push({
      kind,
      startMs,
    ...
    if (convectiveWindow?.startTime) {
      let stormStartIndex = convectiveWindow.startIndex;
      let stormEndIndex = convectiveWindow.endIndex;
      let stormLikelihood = convectiveWindow.convective?.level === "likely" ? "likely" : "possible";
      const overlapStart = Math.max(stormStartIndex, precipWindow.startIndex);
      const overlapEnd = Math.min(stormEndIndex, precipWindow.endIndex);
      if (overlapStart <= overlapEnd) {
        const overlapHours = Array.from({ length: overlapEnd - overlapStart + 1 }, (_, offset) => (
          forecastHourPresentation(data, overlapStart + offset, {
            isCurrent: overlapStart + offset === currentIndex,
            truth: overlapStart + offset === currentIndex ? truth : null
          })
        ));
        const firstLikely = overlapHours.find((hour) => hour.convective?.level === "likely");
        stormStartIndex = firstLikely?.index ?? overlapStart;
        stormEndIndex = overlapEnd;
        stormLikelihood = firstLikely ? "likely" : "possible";
      }
      const stormStartMs = parseForecastTimestamp(data?.hourly?.time?.[stormStartIndex], data);
      const stormEndMs = data?.hourly?.time?.[stormEndIndex + 1]
        ? parseForecastTimestamp(data.hourly.time[stormEndIndex + 1], data)
        : stormStartMs + 60 * 60 * 1000;
      const stormTiming = nearcastRelativeTiming(data, stormStartMs, stormEndMs);
      const stormHeadline = `Storms ${stormLikelihood} ${stormTiming.rangeLabel}`;
      peakStartMs = stormStartMs;
      peakEndMs = stormEndMs;
      peakStartIndex = stormStartIndex;
      peakEndIndex = stormEndIndex;
      phases.push({
        kind: "storm",
        startMs: stormStartMs,
        endMs: stormEndMs,
        startIndex: stormStartIndex,
        endIndex: stormEndIndex,
        likelihood: stormLikelihood,
        chance: null,
        headline: stormHeadline
      });
      if (stormStartMs >= startMs - 30 * 60 * 1000 && stormStartMs <= startMs + 30 * 60 * 1000) {
        kind = "storm";
        likelihood = stormLikelihood;
        headline = stormHeadline;
      } else {
        support = stormHeadline;
      }
    }
  } else {
    phases.push({ kind, startMs, endMs, startIndex, endIndex, likelihood, chance, headline });
  }

  const timing = nearcastRelativeTiming(data, startMs, endMs);
  const focusIndex = Number.isInteger(startIndex) ? startIndex : base.hourlyIndex;
  const event = {
    ...base,
    kind,
    headline: nearcastBriefCleanSentence(headline),
    label: nearcastBriefCleanSentence(headline),
    support: support ? nearcastBriefCleanSentence(support) : null,
    startMs,
    endMs,
    peakStartMs,
    peakEndMs,
    ...
    likelihood,
    phases,
    active: timing.dayRelation === "now",
    target: launchDetailTarget(data, "Forecast change", nearcastBriefCleanSentence(headline), startMs, {
      endMs,
      hours: Math.max(1, Number.isInteger(endIndex) && Number.isInteger(startIndex) ? endIndex - startIndex + 1 : 1)
    })
  };
  event.requiresJump = currentIndex >= 0 && focusIndex >= 0 && focusIndex - currentIndex >= Math.max(1, Number(options.visibleHourCount) || 5);
  event.id = forecastMaterialEventId(data, event, truth);
  return event;
}
...
function buildNearcastBrief(data, options = {}) {
  ...
  const condition = truth.label || weatherCodes[truth.nowCode ?? truth.code] || "Weather";
...
// The shared presentation contract is the only package UI surfaces need:
// one live current snapshot, one reconciled condition per hour/day, one story,
// and one evidence receipt. Raw provider codes stay inside this layer.
function buildForecastPresentation(data, options = {}) {
  const truth = options.truth || buildWeatherTruth(data);
  const tempUnit = options.tempUnit || (state.unit === "fahrenheit" ? "F" : "C");
  const windUnit = options.windUnit || (state.unit === "fahrenheit" ? "mph" : "km/h");
  const currentIndex = currentHourlyIndex(data);
  ...
  for (let index = Math.max(0, currentIndex); index < hourlyCount && hours.length < 25; index += 1) {
    hours.push(forecastHourPresentation(data, index, {
      isCurrent: index === currentIndex,
      truth: index === currentIndex ? truth : null
    }));
  }
  ...
  if (materialEvent) {
    hours = hours.map((hour) => {
      const phase = materialEvent.phases?.slice().reverse().find((item) => (
        Number.isFinite(Number(item.startMs)) && Number.isFinite(Number(item.endMs)) &&
        Number(hour.ms) >= Number(item.startMs) && Number(hour.ms) < Number(item.endMs)
      )) || null;
      const inside = Number(hour.ms) >= Number(materialEvent.startMs) && Number(hour.ms) < Number(materialEvent.endMs);
      return {
        ...hour,
        eventId: inside ? materialEvent.id : null,
        eventPhase: phase?.kind || (inside ? materialEvent.kind : null),
        eventStarts: Boolean(phase && hour.index === phase.startIndex),
        eventEnds: Boolean(phase && hour.index === phase.endIndex)
      };
    });
  }
  return {
    ...
    currentIndex,
    todayIndex,
    current: truth.current || canonicalCurrentSnapshot(data),
    truth,
    hours,
    days,
    story,
...
function renderTodayGlance(data, tempUnit, windUnit, todayIndex = forecastDailyIndex(data), truth = weatherTruth(data), presentation = null) {
  const current = truth.current || canonicalCurrentSnapshot(data);
  ...
  if (els.forecastPulse) els.forecastPulse.hidden = true;
...
function renderForecastPulse(data = state.forecast, dayIndex = forecastDailyIndex(data)) {
  ...
  const visible = pulse.status === "shifting" || pulse.status === "uncertain";
...
function renderNearcastBriefJump(brief) {
  ...
  const label = `${event.timing.timeLabel} ${event.timing.dayLabel}`;
...
function syncNativeWidgetSnapshot(data = state.forecast, place = state.activePlace, truth = state.weatherTruth || weatherTruth(data)) {
  ...
  try {
    ...
    const materialEvent = forecastMaterialEvent(data, { truth, tempUnit, windUnit });
    const provenance = forecastProvenance(data);
    const confidenceAllowedForCompanion = !provenance.cacheFallback && (
    ...
    const snapshot = {
      ...
      canonicalEventId: materialEvent?.id || null,
      canonicalEventHeadline: materialEvent?.headline || null,
      canonicalEventTiming: materialEvent?.timing?.rangeLabel || null,
...
function nativeWidgetDaily(data = state.forecast) {
  ...
  for (let index = startIndex; index < dates.length && rows.length < 4; index += 1) {
    const presentation = forecastDayPresentation(data, index);
...
function nativeWeatherCode(code) {
  const value = Number(code);
  if (!Number.isFinite(value)) return 0;
  return value === RAIN_LIKELY_CODE ? 61 : Math.round(value);
}
...
function buildAIContext(data = state.forecast, place = state.activePlace, alertsSource = activeAlerts) {
  ...
  // Reuse the 15-minute analysis only when it agrees with canonical current
  // truth. Radar/current guidance can establish active precipitation even when
  // a sparse nowcast row appears dry.
  ...
  if (truth?.nowPrecip?.isWetNow && (!nc || ncConflicts)) {
    ...
    nowcast = "No precipitation is indicated in the available near-term forecast";
  } else {
    nowcast = truth?.precip?.detail || truth?.receiptDetail || "No 15-minute precipitation forecast is available";
  }
  const air = airQualitySummary(data);

  // Sparse hourly points (~every 3h) so the model can answer "later today".
  let start = currentHourlyIndex(data);
  if (start < 0) start = 0;
  const next12h = [];
  for (let k = 0; k < 5; k++) {
    const i = start + k * 3;
    if (i >= hourly.time.length) break;
    const hour = forecastHourPresentation(data, i, { isCurrent: i === start, truth: i === start ? truth : null });
    next12h.push({
  ...
  const dailyArr = [];
  const todayIndex = forecastDailyIndex(data);
  for (let i = todayIndex; i < daily.time.length && dailyArr.length < 10; i++) {
    const day = forecastDayPresentation(data, i);
    const offset = i - todayIndex;
    dailyArr.push({
      date: daily.time[i],
      dow: new Date(`${daily.time[i]}T12:00:00`).getDay(),
      label: offset === 0 ? "Today" : offset === 1 ? "Tomorrow" : dayName(daily.time[i]),
  ...
  return {
    ...
    asOf: formatForecastMs(now, data),
    ...
    now: {
      ...
      sky: truth.label || sky(truth.nowCode ?? cur.weather_code),
    ...
    nextMeaningfulEvent: materialEvent ? {
      id: materialEvent.id,
      kind: materialEvent.kind,
      headline: materialEvent.headline,
      support: materialEvent.support || null,
      startsAt: materialEvent.startMs,
      endsAt: materialEvent.endMs,
      timing: materialEvent.timing?.rangeLabel || "",
      likelihood: materialEvent.likelihood,
      basis: materialEvent.basis,
      source: materialEvent.source
    } : null,
    today: {
      hi: r(daily.temperature_2m_max[todayIndex]),
      lo: r(daily.temperature_2m_min[todayIndex]),
      rainChance: daily.precipitation_probability_max[todayIndex] || 0,
      uvPeak: r(daily.uv_index_max[todayIndex]),
      sunrise: daily.sunrise[todayIndex] ? shortClock(daily.sunrise[todayIndex]) : null,
      sunset: shortClock(daily.sunset[todayIndex])
    },
    tomorrow: {
      hi: r(daily.temperature_2m_max[todayIndex + 1]),
      lo: r(daily.temperature_2m_min[todayIndex + 1]),
      rainChance: daily.precipitation_probability_max[todayIndex + 1] || 0,
      sky: forecastDayPresentation(data, todayIndex + 1).label,
      timing: forecastDayPresentation(data, todayIndex + 1).timing || "",
      sunrise: daily.sunrise[todayIndex + 1] ? shortClock(daily.sunrise[todayIndex + 1]) : null,
      sunset: daily.sunset[todayIndex + 1] ? shortClock(daily.sunset[todayIndex + 1]) : null
    },
    next12h,
    daily: dailyArr,
    air,
    alerts,
    forecastStatus: {
      checkedAt: provenance.savedAt,
      ageMinutes: provenance.savedAt ? Math.max(0, Math.round((Date.now() - provenance.savedAt) / 60000)) : null,
      cacheFallback: provenance.cacheFallback,
      precipitationBasis: truth.precip?.basis || (truth.source === "radar-current" ? "observed" : "forecast"),
      precipitationSource: truth.precip?.source || truth.source || "forecast",
      alertsState: typeof alertTrustState === "undefined" ? "unknown" : alertTrustState.state
    }
  };
}
...
function buildAIFactSheet() {
  ...
  if (c.forecastStatus?.cacheFallback) {
    lines.push(`Freshness: this is a saved forecast from ${c.forecastStatus.ageMinutes ?? "an unknown number of"} minutes ago because the live refresh failed. Say so when freshness matters.`);
  } else if (c.forecastStatus?.ageMinutes !== null) {
    lines.push(`Freshness: forecast checked ${c.forecastStatus.ageMinutes} minutes ago.`);
  }
  if (c.forecastStatus?.precipitationBasis === "observed") {
    lines.push("Precipitation evidence: current precipitation is confirmed by radar.");
  } else {
    lines.push("Precipitation evidence: current and near-term precipitation values are modeled forecast guidance, not direct observations.");
  }
  ...
  if (c.nextMeaningfulEvent) {
    const event = c.nextMeaningfulEvent;
    lines.push(`Next meaningful weather: ${event.headline}${event.support ? `. ${event.support}` : ""}. Use this exact event timing when answering follow-up questions.`);
  }
  ...
  if (c.now.isDay) {
    ...
    lines.push(
      ...
      `low ${c.tomorrow.lo}${u.temp}, ${rainWord(c.tomorrow.rainChance)}${c.tomorrow.timing ? `; ${c.tomorrow.timing.toLowerCase()}` : ""}.`
    );
  } else {
    ...
    lines.push(
      ...
      `Tomorrow: ${c.tomorrow.sky.toLowerCase()}, high ${c.tomorrow.hi}${u.temp}, ` +
      `${rainWord(c.tomorrow.rainChance)}${c.tomorrow.timing ? `; ${c.tomorrow.timing.toLowerCase()}` : ""}. UV index peaks at ${c.today.uvPeak}.`
    );
  }

  if (c.next12h && c.next12h.length) {
    const pts = c.next12h
      .map((h) => `${h.t} ${h.temp}${u.temp}, ${h.sky.toLowerCase()}${h.rain >= 30 ? ` (${h.rain}% precipitation)` : ""}`)
      .join(", ");
  ...
  return [
    {
      ...
      value: morningRain >= 40
        ...
        : `Dry through midday. ${weatherCodes[weatherTruth(data).nowCode] || "Mild"} conditions.`
    },
    {
...
function hourlyMetricValue(data, index, metric, isCurrent = false, truth = null, presentation = null) {
  ...
  if (metric === "wind") {
    const value = isCurrent
      ? current?.wind_speed_10m ?? data.hourly.wind_speed_10m?.[index]
  ...
  if (metric === "feels") {
    const value = isCurrent
      ...
      : data.hourly.apparent_temperature?.[index] ?? data.hourly.temperature_2m?.[index];
    return Math.round(Number(value) || 0);
  }
  const value = isCurrent ? current?.temperature_2m : data.hourly.temperature_2m?.[index];
  return Math.round(Number(value) || 0);
}
...
function uvForecastInsight(data, dayIndex = forecastDailyIndex(data)) {
  ...
  const hasClearSkyReference = Number.isFinite(clearSkyPeak) && clearSkyPeak > 0;
  const cloudReduction = hasClearSkyReference
    ? Math.max(0, Math.round((1 - forecastPeak / clearSkyPeak) * 100))
    : null;
...
function renderHourly(data, tempUnit, truth = weatherTruth(data), presentation = null, options = {}) {
  ...
  const cards = rows.map(({ time, index }, position) => {
    ...
    const metricValue = metricValues[position];
    const gust = Math.max(0, Math.round(Number(hour.gust) || 0));
    const label = position === 0 ? "Now" : formatHour(time);
    const showStormPotentialBadge = stormPotential && !isThunderCode(wcode) && (
      !canonicalEvent || hour.eventStarts || index === canonicalEvent.peakStartIndex
    );
    const thunderLabel = convective?.label || "Thunder possible";
    const title = showStormPotentialBadge ? `${code}; ${thunderLabel.toLowerCase()}` : code;
    ...
    return `
      <article class="hour-card metric-${metric}${position === 0 ? " current" : ""}${isStoryFocus ? " story-focus" : ""}${showStormPotentialBadge ? " has-storm-potential" : ""}${hasPlanMemory ? " has-plan-memory" : ""}" style="--trend-y:${Number.isFinite(trend.points[position].y) ? trend.points[position].y.toFixed(1) : "25"}px" role="button" tabindex="0" data-hour-index="${index}" aria-label="${escapeHtml(cardLabel)}" title="${escapeHtml(title)}">
        ...
        <strong class="hour-temp hour-trend-value"${temperatureColorStyle}>${metricPresentation.trendLabel}</strong>
        <span class="hour-condition" aria-hidden="true">${escapeHtml(cardCondition)}</span>
        <span class="hour-secondary${metric === "temperature" ? rainClass : ""}">${metricPresentation.secondary}</span>
      </article>
    `;
  }).join("");
  els.hourly.innerHTML = `
...
function outdoorWindowTime(ms, data) {
  return formatForecastMs(ms, data);
}

function outdoorWindowDayIndex(data, ms) {
  const date = forecastLocalDateFromMs(ms, data);
  const index = data?.daily?.time?.findIndex((value) => value === date) ?? -1;
...
function outdoorWindowCandidate(data) {
  ...
  const candidates = rows.map((first, index) => {
    ...
    const gustMph = windUnit === "mph" ? gust : gust / 1.60934;
    ...
    return {
      ...
      uv,
      isDay: windowRows.some((row) => row.isDay),
      score: score + (alertBlocksRecommendation ? 1000 : 0),
      good,
      standout,
      officialAlert,
...
function outdoorWindowTimeRange(window, data) {
  const startDay = forecastLocalDateFromMs(window.startMs, data);
  const endDay = forecastLocalDateFromMs(window.endMs, data);
  const day = data?.daily?.time?.findIndex((value) => value === startDay) ?? -1;
  const prefix = day >= 0 ? `${formatDay(startDay, day)} · ` : "";
  const endPrefix = startDay === endDay ? "" : `${formatDay(endDay, day + 1)} `;
  return `${prefix}${outdoorWindowTime(window.startMs, data)}–${endPrefix}${outdoorWindowTime(window.endMs, data)}`;
}

function outdoorWindowWindCopy(window) {
  const unit = window.windUnit;
  const wind = window.wind;
  const gust = window.gust;
  const gentleThreshold = unit === "mph" ? 6 : 10;
  const breezeThreshold = unit === "mph" ? 11 : 18;
  if (gust > (unit === "mph" ? 20 : 32)) return `gusts ${gust} ${unit}`;
  if (wind <= gentleThreshold) return "calm wind";
  if (wind <= breezeThreshold) return "light breeze";
  return `wind ${wind} ${unit}`;
}

function outdoorWindowCopy(window) {
  ...
  return `${capitalize(rain)}, ${wind}, and ${uv}.`;
}
...
function renderGoodOutdoorWindow(data) {
  ...
  slot.hidden = false;
  slot.innerHTML = `
    <section class="outdoor-window-card${window.good ? "" : " is-caution"}" aria-label="${escapeHtml(`${kicker}: ${heading}`)}">
      <div class="outdoor-window-head">
        <span class="outdoor-window-kicker">${escapeHtml(kicker)}</span>
      ...
      <button class="outdoor-window-detail" type="button" data-good-window-start="${window.startMs}" data-good-window-end="${window.endMs}" data-good-window-day="${window.dayIndex}" aria-label="${escapeHtml(aria)}">
        ...
        <span class="outdoor-window-open">Hourly <i class="outdoor-window-arrow" aria-hidden="true">›</i></span>
      </button>
    </section>
  `;
}

function openGoodOutdoorWindowDetail(button) {
  const data = state.forecast;
  if (!data || !button) return;
  const startMs = Number(button.dataset.goodWindowStart);
  const endMs = Number(button.dataset.goodWindowEnd);
  const dayIndex = Number(button.dataset.goodWindowDay);
  if (!Number.isFinite(startMs) || !Number.isFinite(endMs) || !Number.isInteger(dayIndex)) return;
  openDayFromIndex(dayIndex, {
    ...
    eventWindow: {
      startMs,
      endMs,
      badgeLabel: "Best window",
      label: "Suggested outdoor window"
    },
    contextLabel: "Suggested outdoor window"
  });
}
...
function renderDaily(data, tempUnit, precipUnit, presentation = null) {
  ...
  const highs = data.daily.temperature_2m_max;
  const lows = data.daily.temperature_2m_min;
  const visibleHighs = highs.slice(todayIndex);
  const visibleLows = lows.slice(todayIndex);
  const minTemp = Math.min(...visibleLows);
  ...
  const dayRows = data.daily.time.map((time, index) => {
    ...
    return `
      <article class="day-row${index === todayIndex ? " current" : ""}${stormPotential ? " has-storm-potential" : ""}${memoryCue ? " has-plan-memory" : ""}" data-index="${index}" role="button" tabindex="0" aria-label="${escapeHtml(dayAria)}">
        <div class="day-label">
          <div class="day-icon weather-icon-with-badge" aria-hidden="true">${weatherIcon(wcode, true, { density: "dense" })}${stormPotential ? thunderBadgeHtml(stormLabel) : ""}</div>
          <div>
...
function buildFeelsGlanceDetail(data, tempUnit, windUnit) {
  ...
  return {
    ...
    body: `
      ...
      <div class="glance-detail-facts">
        ${glanceDetailFactHtml("Air temp", `${actual}${degree(tempUnit)}`, "Current forecast estimate")}
...
function formatDay(value, index) {
  const relativeIndex = state.forecast ? daysFromForecastToday(value, state.forecast) : index;
  if (relativeIndex === 0) return "Today";
  if (relativeIndex === 1) return "Tomorrow";
...
const NWS_COUNTRY_CODES = new Set(["US", "PR", "GU", "VI", "AS", "MP"]);
let activeAlerts = [];
...
function placeSupportsNwsAlerts(place = state.activePlace) {
  const code = placeCountryCode(place);
  if (code) return NWS_COUNTRY_CODES.has(code);
...
function updateAlertTrustState(nextState, reason = "") {
  alertTrustState = {
    state: nextState,
    checkedAt: ["ready", "failed"].includes(nextState) ? Date.now() : null,
...
async function startNwsConvectiveProbe(place, data, force = false) {
  ...
  try {
    ...
    renderForecast(data, place, {
      ...
      // as the live hour, so keep the map and continuity lanes quiet but
      // refresh every forecast surface that presents conditions.
      skip: ["map", "continuity"],
...
function setAlertsLoading(place = state.activePlace) {
  activeAlerts = [];
  activeAlertsReady = false;
  updateAlertTrustState(placeSupportsNwsAlerts(place) ? "checking" : "unsupported");
  const bar = document.getElementById("alertBar");
...
function setAlertsFailed(error = null) {
  activeAlerts = [];
  activeAlertsReady = false;
  updateAlertTrustState("failed", error?.name || error?.message || "alert-fetch-failed");
  const bar = document.getElementById("alertBar");
  if (bar) bar.hidden = true;
  syncLaunchAlertReadingOrder(false);
  clearLaunchAlertAnnouncement(false);
...
function syncLaunchAfterAlertsReady() {
  if (!state.forecast || !state.activePlace) return;
  renderForecastTrust(state.forecast, state.weatherTruth || weatherTruth(state.forecast));
