...
- Raw radar history is bounded to six actual scans over thirty minutes, with a six-entry/12 MiB rendered-frame LRU per source. The September 20 coverage-padding refinement increased the former eight-MiB budget to keep all six 512×672 frames resident (12,386,304 bytes including pixel/coverage values). The two rendered caches total at most 24 MiB, excluding other app/model/decoder memory. Replaying a nearby viewport reuses images and their coverage evidence; only changed coverage envelopes or render-quality buckets clear them. Expired exact selections clear their bitmap before request deduplication. Initial and manual metadata work cancel on suspension.
