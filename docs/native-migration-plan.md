...
[Phase 3A's native radar foundation](native-radar-foundation.md) now adds a separate checksum-pinned MapLibre prototype, exact-time/source timeline policy, and matched synthetic numerical fixtures. It is not linked into Nearcast or TestFlight and does not replace the Map tab. Compilation, deterministic timeline/numeric tests and isolated simulator installation/launch are verified; visual gestures are blocked by the locked Mac, and full enhanced-map parity, provider authorization and physical performance remain open. Continue with those gates before promoting a native radar checkpoint.
