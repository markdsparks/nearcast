...
[Phase 2B2 / build 105](native-places-ownership-testflight-105.md) implements the explicit native Places/Settings ownership handover, independent native lookup and editing, compatibility routing for existing screens, and revision-checked widget/Watch publication. Automated suites and isolated simulator checks cover the transition and editing without a loaded web document. The next family checkpoint remains opt-in; physical acceptance is still open. Activating native ownership requires build 105 or a newer compatible reader; returning to the existing weather screen is supported, but downgrading to an older binary is not a data rollback. Native radar is the next migration surface, not part of this checkpoint.

[Phase 3A's native radar foundation](native-radar-foundation.md) adds a checksum-pinned MapLibre prototype, exact-time/source timeline policy, and matched synthetic numerical fixtures. At the user's request, [build 106](native-radar-lab-testflight-106.md) makes this an explicit Radar Lab opt-in from native Settings, without replacing the Map tab. Earlier compilation, deterministic timeline/numeric tests and isolated simulator installation/launch are verified; visual gestures remain unverified while the Mac is locked. Early family testing may proceed in parallel, but full enhanced-map parity, provider authorization, visual QA and physical performance remain open before a native map can become the default.

After the Mac was unlocked, the [native map preview checkpoint](native-map-preview-checkpoint.md) adds actual native MRMS/HRRR readers, global observed fallback, selected-place presentation, source-safe timeline controls and an isolated iOS basemap authorization lane. Initial simulator inspection exposed and fixed the Settings presentation race. This is still Phase 3 in progress: CARTO iOS authorization, physical gestures/performance, seam/subhourly logic and remaining required layers are explicit open gates. The existing Map tab remains unchanged; no newer TestFlight upload is implied by this code checkpoint.
