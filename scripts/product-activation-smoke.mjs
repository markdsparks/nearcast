...
assert.match(extractFunction(app, "arrangeForecastHierarchy"), /launch\.after\(nowcast, hourlyPanel, dailyPanel, map, essentials, extendedDailyPanel, els\.familyPlacesPeek, els\.planPulse, els\.goodWindow/, "the universal forecast and essentials scan stays ahead of extended detail and earned family-place exceptions");
...
assert.match(html, /id="hourlyIntervalTabs"[\s\S]*data-hourly-interval="hourly"[\s\S]*data-hourly-interval="quarter-hour"[\s\S]*Next 6 hours/, "the hero offers a distinct, bounded 15-minute mode instead of implying day-long precision");
...
assert.match(extractFunction(app, "rebuildForecastTemperatures"), /NearcastSharedForecast\.applyTemperatureGuidance[\s\S]*forecastTemperatureGuidanceByForecast[\s\S]*nwsConvectiveEvidenceByForecast/, "Home and native refreshes use one temperature calibration implementation with model and NWS guidance");
