...
assert.match(plannerSource, /id: "nearcast\.forecast_confidence"/);
assert.match(plannerSource, /produces: \["nearcast\.forecast-confidence"\]/);
assert.match(plannerSource, /never changes a plan verdict, watch state, or notification policy/);
assert.match(plannerSource, /function nearcastExplicitConfidenceQuestion/);
assert.match(plannerSource, /function nearcastConfidenceAnswerMessage[\s\S]*?No meaningful timing change/);
assert.match(plannerSource, /required_skill_ids: \["nearcast\.forecast_confidence"\]/);
assert.match(plannerSource, /window\.nearcastForecastConfidenceForWindow\(\{[\s\S]*?startMs: target\.startMs,[\s\S]*?endMs: target\.endMs/);
assert.match(plannerSource, /function nearcastCalmForecastAdvice[\s\S]*?Keep it flexible/);
assert.match(plannerSource, /forecastDisclosurePresentation\(\{[\s\S]*?decisionSensitive: true/);
assert.match(plannerSource, /function planUncertaintyAdviceForEvent[\s\S]*?nearcastCalmForecastAdvice/);
assert.match(plannerSource, /function planWindowUncertaintyAdvice/);
assert.match(plannerSource, /const adjustment = mergePlanActionAdvice\(hint, forecastAdvice\)/);
assert.match(plannerSource, /const action = mergePlanActionAdvice\(primaryAction, forecastAdvice\)/);
assert.doesNotMatch(plannerSource, /Confidence note:/, "ordinary plan answers never append a technical confidence paragraph");
assert.doesNotMatch(plannerSource, /<span>Confidence<\/span>/, "plan cards fold actionable uncertainty into one advice row");
assert.match(await readFile(new URL("../app.js", import.meta.url), "utf8"), /window\.nearcastForecastConfidenceForWindow = nearcastForecastConfidenceForWindow/);
...
assert.ok(plannerSource.includes("mid\\s*day"), "continuous spans understand midday boundaries");
assert.match(plannerSource, /context\.receipt\.schedule = schedule/);
...
assert.match(plannerSource, /id: "nearcast\.plan_watch"/);
assert.match(plannerSource, /id: "nearcast\.plan_unwatch"/);
assert.match(plannerSource, /id: "nearcast\.plan_edit"/);
assert.match(plannerSource, /id: "nearcast\.plan_make_weekly"/);
assert.match(plannerSource, /executeNearcastPlanMakeWeeklySkill/);
assert.match(plannerSource, /requires_user_confirmation: true,[\s\S]*consumes: \["nearcast\.plan-watch"\]/);
assert.match(plannerSource, /id: "nearcast\.settings_update"/);
assert.match(plannerSource, /id: "nearcast\.navigate"/);
